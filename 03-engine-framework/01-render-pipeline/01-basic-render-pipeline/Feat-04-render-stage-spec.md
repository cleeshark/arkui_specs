...
**As a** 渲染管线维护者
**I want** 子节点增删移、Z 序变化、可见性/转场状态变化能通过 MarkNeedSyncRenderTree → RebuildRenderContextTree 链路同步到 RS 渲染节点树
**So that** 渲染结果反映最新的逻辑节点树状态
...
**As a** 渲染管线维护者
**I want** FrameNode 的 `isPendingState_` 标记按现有 Attach/Detach 调用规则配对
**So that** 依赖 `OnAttachToMainRenderTree`/`OnDetachFromMainRenderTree` 的功能获得一致的生命周期通知
...
**As a** 渲染管线维护者
**I want** 明确 ProcessRenderTreeDiff 中 Attach/Detach 的现有调用时序和参数传递
**So that** 下游变更能够识别同帧跨父节点重挂载时的已知状态偏差
...
**As a** 渲染管线维护者
**I want** GenerateOneDepthVisibleFrameWithTransition 按现有规则收集可见子节点和转场中的子节点
**So that** 转出动画节点在动画完成前保留在渲染树中，其他不参与渲染的节点被排除
...
**As a** 渲染管线维护者
**I want** frameChildren_ 按 ZIndex 排序，并由 RS 子树重建链路执行增量 diff
**So that** 子节点 Z 序变化反映到渲染结果，同时避免无条件全量重建
...
**As a** 应用开发者
**I want** 通过 UIContext 的 nodeRenderState API 监听指定节点的渲染状态变化
**So that** 在节点进入或离开当前实现判定的渲染状态时执行相应逻辑
...
**As a** 渲染管线维护者
**I want** dirty render 节点在 `UITaskScheduler::FlushTask` 的 Render 阶段被一次性排空，并由 `PaintWrapper::FlushRender` 在 Modifier 与 Draw 之间选择路径
**So that** 布局完成后的绘制结果稳定提交到 RS，且属性动画不会重复录制 Draw
