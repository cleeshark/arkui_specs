...
- **作为** 渲染管线维护者
- **我想要** 每帧仅在 `FlushMessages → SendMessages` 一处提交 RS，其后修改进入下一帧
- **以便** "帧完成"具备唯一可观察边界，避免无意识跨帧
...
- **作为** 性能维护者
- **我想要** 同一帧内多次 `RequestFrame` 自动合并为一次 VSync 请求，且后台窗口默认不请求 VSync
- **以便** 减少 IPC 频次并避免后台无意义绘制
...
- `foundation/graphic/graphic_2d/rosen/modules/render_service_client/core/ui/rs_canvas_node.cpp:98-120,149-170,186-215`（BeginRecording / FinishRecording / DrawOnNode）
