...
- Every Claim review must have at least one atomic `unit_reviews` row. Keep `reviewed_units` and
  `unit_reviews` in the same non-empty order, with one row per independently verifiable facet; a
  simple Claim still gets one unit, while a compound Claim must be split rather than covered by a
  blanket unit.
