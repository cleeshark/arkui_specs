...
|-------|-------------|---------|---------|
| VM-1 | US-1 创建空白占位组件 (AC-1.1~1.6) | 单元测试 + 代码审查 | FrameNode 创建；Flex 属性默认值 |
| VM-2 | US-2 背景颜色 (AC-2.1~2.4) | 单元测试 + 代码审查 | color 设置/主题跟随；配置变更回调路径 |
| VM-3 | US-3 显式高度 (AC-3.1~3.3) | 单元测试 + 代码审查 | min 约束；blank_modifier C API 路径 |
| VM-4 | US-4 自动 Flex 布局 (AC-4.1~4.8) | 单元测试 + 代码审查 | API≥10 自动填充；API<10 静态 Flex；负值 min 钳位 |
