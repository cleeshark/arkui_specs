...
|----|----------|-----------|----------|------|
| AC-1.1~1.6 | FR-1, BR-1 | N/A（存量） | 单测 | `test/unittest/core/pattern/text/` |
| AC-2.1~2.5 | FR-2, FR-3, EX-1 | N/A | 单测 | 同上 |
| AC-3.1~3.4 | FR-4, BR-2, EX-2 | N/A | 单测 | 同上 |
| AC-4.1~4.3 | FR-5, EX-3 | N/A | 单测 | 同上 |
| AC-5.1~5.3 | FR-6, EX-3 | N/A | 单测 | 同上 |
| AC-6.1~6.4 | FR-7, BR-3 | N/A | 单测 | 同上 |
| AC-7.1~7.2 | FR-8 | N/A | 单测 | 同上 |
| AC-8.1~8.2 | FR-9 | N/A | 单测 | 同上 |
| AC-9.1~9.6 | FR-10, EX-4 | N/A | 单测 | 同上 |
| AC-10.1~10.5 | FR-11, BR-4 | N/A | 单测 | 同上 |
| AC-11.1~11.5 | FR-12, BR-4, BR-5 | N/A | 单测 | 同上 |
| AC-12.1~12.4 | FR-13 | N/A | 单测 | 同上 |
| AC-13.1~13.5 | FR-14, BR-1 | N/A | 单测 | 同上 |
| AC-14.1~14.5 | FR-15 | N/A | 单测 | 同上 |

## 业务规则

| 编号 | 规则描述 | 约束条件 | 关联 AC |
|------|----------|----------|---------|
| BR-1 | lineHeight 正值启用"高度覆盖"模式，影响自适应 maxLines 的行数推断 | lineHeight > 0 时 `hasHeightOverride_=true`（`text_style.h:809`） | AC-1.6, AC-13.4 |
| BR-2 | lineHeightMultiple 与 lineHeight 隐式耦合：设置 lineHeightMultiple 时会先将 lineHeight 强制设为 28px（DEFAULT_LINE_HEIGHT） | JS Bridge（`js_text.cpp:647`）、Static 版（`text_model_static.cpp:539`）、C API（`style_modifier.cpp:14083`）三条路径均如此 | AC-3.3, AC-3.4 |
| BR-3 | halfLeading 默认值来源于 PipelineContext 全局配置 | 组件未设置时从 `pipeline->GetHalfLeading()` 取值（`multiple_paragraph_layout_algorithm.cpp:186`），而非硬编码 false | AC-6.4 |
| BR-4 | textContentAlign 动态版默认 CENTER、静态版默认 TOP | 动态版 `text_model_ng.cpp:1307` 默认 `TextContentAlign::CENTER`；静态版 `text.static.d.ets` 标注默认 `TOP` | AC-11.4, AC-10.5 |
| BR-5 | textContentAlign 使用 PROPERTY_UPDATE_LAYOUT 而非 PROPERTY_UPDATE_MEASURE | 变更仅触发重新布局，不触发重新测量（`text_layout_property.h:166`），与大多数属性的 PROPERTY_UPDATE_MEASURE 行为不同 | AC-11.5 |

## 功能规则

| 编号 | 规则描述 | 触发条件 | 作用对象 | 关联 AC |
|------|----------|----------|----------|---------|
| FR-1 | `lineHeight` 设置行高，单位 fp | 设置有效值（number/string/Resource） | `TextLayoutProperty` → TextLineStyle::propLineHeight（`text_styles.h:360`） | AC-1.1~1.6 |
| FR-2 | `lineSpacing` 设置行间距 | 设置 LengthMetrics 正值 | `TextLayoutProperty` → TextLineStyle::propLineSpacing（`text_styles.h:376`） | AC-2.1~2.2 |
| FR-3 | `lineSpacing` + `onlyBetweenLines` 模式 | options.onlyBetweenLines = true (@since 20) | `TextLayoutProperty` → TextLineStyle::propIsOnlyBetweenLines（`text_styles.h:377`） | AC-2.3~2.4 |
| FR-4 | `lineHeightMultiple` 设置行高倍数 | 设置正值 double (@since 22) | `TextLayoutProperty` → propLineHeightMultiply（WITHOUT_GROUP，`text_layout_property.h:194`） | AC-3.1~3.4 |
| FR-5 | `minLineHeight` 设置行高下限 | 设置 LengthMetrics 正值 (@since 22) | `TextLayoutProperty` → propMinimumLineHeight（WITHOUT_GROUP，`text_layout_property.h:195`） | AC-4.1~4.3 |
| FR-6 | `maxLineHeight` 设置行高上限 | 设置 LengthMetrics 正值 (@since 22) | `TextLayoutProperty` → propMaximumLineHeight（WITHOUT_GROUP，`text_layout_property.h:196`） | AC-5.1~5.3 |
| FR-7 | `halfLeading` 启用半行距分配 | 设置 boolean (@since 12) | `TextLayoutProperty` → TextLineStyle::propHalfLeading（`text_styles.h:380`） | AC-6.1~6.4 |
| FR-8 | `fallbackLineSpacing` 启用回退字体行间距 | 设置 boolean (@since 23) | `TextLayoutProperty` → propFallbackLineSpacing（WITHOUT_GROUP TEXT，`text_layout_property.h:200`） | AC-7.1~7.2 |
| FR-9 | `includeFontPadding` 启用字体内边距 | 设置 boolean (@since 23) | `TextLayoutProperty` → propIncludeFontPadding（WITHOUT_GROUP TEXT，`text_layout_property.h:199`） | AC-8.1~8.2 |
| FR-10 | `textAlign` 设置水平对齐方式 | 设置 TextAlign 枚举 | `TextLayoutProperty` → TextLineStyle::propTextAlign（`text_styles.h:365`） | AC-9.1~9.6 |
| FR-11 | `textVerticalAlign` 设置行内垂直对齐 | 设置 TextVerticalAlign 枚举 (@since 20) | `TextLayoutProperty` → TextLineStyle::propTextVerticalAlign（`text_styles.h:366`） | AC-10.1~10.5 |
| FR-12 | `textContentAlign` 设置内容块垂直对齐 | 设置 TextContentAlign 枚举 (@since 21) | `TextLayoutProperty` → TextLineStyle::propTextContentAlign（`text_styles.h:387`） | AC-11.1~11.5 |
| FR-13 | `textIndent` 设置首行缩进 | 设置 Length 值 (@since 10) | `TextLayoutProperty` → TextLineStyle::propTextIndent（`text_styles.h:371`） | AC-12.1~12.4 |
| FR-14 | `baselineOffset` 设置基线偏移 | 设置 number/ResourceStr 值 | `TextLayoutProperty` → TextLineStyle::propBaselineOffset（`text_styles.h:363`） | AC-13.1~13.5 |
| FR-15 | `textDirection` 设置文本方向 | 设置 TextDirection 枚举 (@since 23) | `TextLayoutProperty` → TextLineStyle::propTextDirection（`text_styles.h:388`） | AC-14.1~14.5 |

## 异常/豁免规则

| 编号 | 规则描述 | 触发条件 | 处理结果 | 关联 AC |
|------|----------|----------|----------|---------|
| EX-1 | lineSpacing 负值处理 | 传入负值 LengthMetrics | JS Bridge 层 Reset 为默认值 0；C API 层 Reset 为默认且返回错误码 | AC-2.2 |
| EX-2 | lineHeightMultiple 负值处理 | 传入 ≤ 0 的值 | JS Bridge 调用 `ResetLineHeightMultiply()`；C API 先 resetMultiply 再返回错误码 | AC-3.2 |
| EX-3 | minLineHeight/maxLineHeight 负值处理 | 传入负值 LengthMetrics | JS Bridge 调用 Reset；C API 设为 0 并返回错误码 | AC-4.2, AC-5.2 |
| EX-4 | textAlign 非法枚举值 | 传入超出 [0, 5] 范围的枚举值 | API ≥ 12 时回退到 `TextAlign.Start`（0）；API < 12 时不生效（`js_text.cpp:560-568`） | AC-9.6 |
| EX-5 | baselineOffset 百分比值 | 传入百分比字符串 | 使用默认值 0（百分比不支持） | AC-13.5 |
| EX-6 | textDirection 非法枚举值 | 传入超出 [0, 3] 范围的值 | C API 调用 resetTextDirection 并返回错误码（`style_modifier.cpp:13673-13686`）；JS Bridge 调用 `ResetTextDirection()` | AC-14.4 |
| EX-7 | halfLeading 非布尔值 | 传入非布尔类型参数 | 设为 false（`js_text.cpp:1326-1337`） | AC-6.2 |

## 恢复契约

| 编号 | 触发条件 | 恢复策略 | 恢复结果 | 约束 |
|------|----------|----------|----------|------|
| RC-1 | TextLineStyle 属性组懒初始化——行布局属性组在首次设置任一行属性时通过 `GetOrCreateTextLineStyle()` 创建 | 未设置任何行布局属性时属性组为 null，各属性按默认值渲染 | 等同于全部使用默认值 | 属性组一旦创建不会被销毁，仅各 optional 字段可被 Reset |
| RC-2 | lineHeightMultiple 被 Reset 时 lineHeight 不会自动恢复 | Reset lineHeightMultiple 仅清除倍数属性 | lineHeight 仍保持先前被隐式设为 28px 的值（如果是通过 lineHeightMultiple 间接设置的） | 需开发者显式设置或 Reset lineHeight 来恢复 |

## 验证映射

| 编号 | 对应规格项 | 验证方式 | 验证重点 |
|------|------------|----------|----------|
| VM-1 | FR-1 (lineHeight) | 单测 | 正值/零值/负值/字符串/Resource 各场景行高渲染正确 |
| VM-2 | FR-2, FR-3 (lineSpacing + onlyBetweenLines) | 单测 | 正值生效、负值回退、onlyBetweenLines 模式差异 |
| VM-3 | FR-4 (lineHeightMultiple) | 单测 | 倍数生效、负值 Reset、对 lineHeight 的隐式副作用 |
| VM-4 | FR-5, FR-6 (minLineHeight/maxLineHeight) | 单测 | 正值夹紧、负值处理、undefined Reset |
| VM-5 | FR-7 (halfLeading) | 单测 | 启用/禁用半行距；pipeline 全局默认值传播 |
| VM-6 | FR-8, FR-9 (fallbackLineSpacing/includeFontPadding) | 单测 | 布尔值生效、多组件类型共享实现 |
| VM-7 | FR-10 (textAlign) | 单测 | 六种对齐方式各自生效、JUSTIFY 配合 wordBreak、非法值回退 |
| VM-8 | FR-11 (textVerticalAlign) | 单测 | 四种垂直对齐方式、静态版缺失情况 |
| VM-9 | FR-12 (textContentAlign) | 单测 | 三种内容对齐方式、PROPERTY_UPDATE_LAYOUT 验证 |
| VM-10 | FR-13 (textIndent) | 单测 | 正值/负值/零值/Resource 各场景 |
| VM-11 | FR-14 (baselineOffset) | 单测 | 正/负偏移、内容高度计算影响、百分比回退 |
| VM-12 | FR-15 (textDirection) | 单测 | LTR/RTL/AUTO/INHERIT 各方向、PROPERTY_UPDATE_MEASURE_SELF 验证 |
