...
|-------|----------|-----------|----------|------|
| AC-1.1 | BR-1, FR-1 | TASK-5 | 单测 | text_pattern_test |
| AC-1.2 | BR-1 | TASK-5 | 单测 | clipboard scope test |
| AC-1.3 | BR-1 | TASK-5 | 单测 | clipboard scope test |
| AC-1.4 | FR-1 | TASK-5 | 单测 | default value test |
| AC-1.5 | BR-2 | TASK-5 | 单测 | marquee + copyOption test |
| AC-1.6 | BR-1, BR-2 | TASK-5 | 单测 | IsSelectableAndCopy test |
| AC-1.7 | FR-2 | TASK-5 | 单测 | SpanString copy test |
| AC-1.8 | FR-2 | TASK-5 | 单测 | plain text copy test |
| AC-1.9 | FR-2 | TASK-5 | 代码审查 | async HTML generation |
| AC-1.10 | FR-3 | TASK-5 | 单测 | keyboard shortcut test |
| AC-1.11 | FR-3 | TASK-5 | 单测 | select all test |
| AC-2.1 | BR-3, FR-4 | TASK-5 | 单测 | textSelectable mode test |
| AC-2.2 | BR-3, FR-4 | TASK-5 | 单测 | focus behavior test |
| AC-2.3 | BR-3, FR-4 | TASK-5 | 单测 | unselectable test |
| AC-2.4 | FR-4 | TASK-5 | 单测 | default mode test |
| AC-2.5 | FR-5 | TASK-5 | 单测 | word boundary test |
| AC-2.6 | FR-5 | TASK-5 | 单测 | double click test |
| AC-2.7 | FR-5 | TASK-5 | 单测 | mouse drag test |
| AC-2.8 | FR-6 | TASK-5 | 单测 | keyboard selection test |
| AC-3.1 | FR-7 | TASK-5 | 单测 | programmatic selection test |
| AC-3.2 | EX-1 | TASK-5 | 单测 | invalid range test |
| AC-3.3 | EX-2 | TASK-5 | 单测 | clamp test |
| AC-3.4 | BR-1, BR-3 | TASK-5 | 单测 | guard condition test |
| AC-3.5 | FR-7 | TASK-5 | 代码审查 | AfterLayoutTask |
| AC-4.1 | FR-8 | TASK-5 | 单测 | selectedBgColor test |
| AC-4.2 | FR-8 | TASK-5 | 单测 | alpha auto-adjust test |
| AC-4.3 | FR-8 | TASK-5 | 单测 | default color test |
| AC-4.4 | FR-9 | TASK-5 | 单测 | caretColor test |
| AC-4.5 | FR-9 | TASK-5 | 单测 | default handle color test |
| AC-4.6 | FR-9 | TASK-5 | 代码审查 | overlay handlerColor |
| AC-5.1 | BR-4, FR-10 | TASK-5 | 单测 | draggable + selection test |
| AC-5.2 | BR-4, FR-10 | TASK-5 | 单测 | long press drag priority |
| AC-5.3 | FR-10 | TASK-5 | 单测 | non-selected area test |
| AC-5.4 | FR-11 | TASK-5 | 单测 | UDMF data format test |
| AC-5.5 | FR-11 | TASK-5 | 单测 | plain text drag test |
| AC-5.6 | RC-1 | TASK-5 | 单测 | drag cancel recovery |
| AC-5.7 | FR-12 | TASK-5 | 单测 | drag preview color test |
| AC-5.8 | FR-12 | TASK-5 | 单测 | default preview color test |
| AC-5.9 | FR-12 | TASK-5 | 代码审查 | drag node layout |
| AC-6.1 | FR-13 | TASK-5 | 单测 | bindSelectionMenu test |
| AC-6.2 | FR-13 | TASK-5 | 单测 | image span menu test |
| AC-6.3 | FR-14 | TASK-5 | 单测 | default wildcard test |
| AC-6.4 | FR-14 | TASK-5 | 单测 | fallback lookup test |
| AC-6.5 | FR-13 | TASK-5 | 单测 | unbind menu test |
| AC-6.6 | FR-15 | TASK-5 | 单测 | onAppear callback test |
| AC-6.7 | FR-15 | TASK-5 | 单测 | onDisappear callback test |
| AC-7.1 | FR-16 | TASK-5 | 单测 | editMenuOptions test |
| AC-7.2 | FR-16 | TASK-5 | 单测 | onMenuItemClick test |
| AC-7.3 | BR-5 | TASK-5 | 代码审查 | overlay update sequence |
| AC-7.4 | BR-5, FR-16 | TASK-5 | 单测 | system menu + editMenuOptions |
| AC-7.5 | FR-17 | TASK-5 | 单测 | menu item visibility test |
| AC-7.6 | FR-17 | TASK-5 | 单测 | select all hide test |
| AC-8.1 | FR-18 | TASK-5 | 单测 | handle move test |
| AC-8.2 | FR-19 | TASK-5 | 代码审查 | vibrator feedback |
| AC-8.3 | FR-20 | TASK-5 | 代码审查 | magnifier show |
| AC-8.4 | FR-20 | TASK-5 | 代码审查 | magnifier hide |
| AC-8.5 | FR-20 | TASK-5 | 代码审查 | magnifier on long press |
| AC-8.6 | FR-18 | TASK-5 | 单测 | external touch test |
| AC-8.7 | EX-3 | TASK-5 | 单测 | handle crossover test |

## 业务规则

| 编号 | 规则 |
|------|------|
| BR-1 | `IsSelectableAndCopy()` 三重门控：`textSelectable != UNSELECTABLE` 且 `copyOption != None` 且 `!textEffect_`（无跑马灯/动效），三者缺一不可。这是选择、复制、拖拽功能的统一入口守卫（`text_pattern.cpp:3816-3822`） |
| BR-2 | `textOverflow` 为 MARQUEE 模式时，`CalcCopyOption()` 强制将 `copyOption_` 设为 `CopyOptions.None`（`text_pattern.cpp:2969`），导致 BR-1 门控失败，选择与复制完全禁用 |
| BR-3 | `TextSelectableMode` 控制选择能力和焦点行为：`SELECTABLE_UNFOCUSABLE`（可选不可焦）、`SELECTABLE_FOCUSABLE`（可选可焦）、`UNSELECTABLE`（不可选）。焦点能力仅在 `SELECTABLE_FOCUSABLE` 模式下启用（`text_pattern.cpp:3801-3813`） |
| BR-4 | 拖拽优先于重新选择：长按已选中文本区域时，`IsDraggable(localOffset)` 检查通过后设置 `gestureHub->SetIsTextDraggable(true)` 并直接返回，阻止 `InitSelectionOnLongPress` 执行（`text_pattern.cpp:718-727`） |
| BR-5 | `bindSelectionMenu` 与 `editMenuOptions` 互补关系：`bindSelectionMenu` 在 `OnUpdateSelectOverlayInfo` 中先执行（line 383），设置 `menuBuilder` 替换整个菜单 UI；`editMenuOptions` 后执行（line 387），设置 `onCreateMenuCallback` 定制系统菜单项。两者同时设置时，自定义 builder 优先渲染，`editMenuOptions` 回调仍挂载但不影响 builder 渲染 |

## 功能规则

| 编号 | 规则 |
|------|------|
| FR-1 | `copyOption` 通过 `ACE_UPDATE_LAYOUT_PROPERTY(TextLayoutProperty, CopyOption, copyOption)` 存储，脏标记为 `PROPERTY_UPDATE_MEASURE`（`text_layout_property.h:207`） |
| FR-2 | 剪贴板写入三种格式：纯文本（`SetPlainText`）+ HTML（`SetHtmlText`，后台线程 `HtmlUtils::ToHtml` 异步生成）+ SpanString TLV 二进制（`EncodeTlv`），通过 `MultiTypeRecordImpl` 打包为 `PasteDataMix`，最终 `clipboard_->SetData(pasteData, copyOption)` 写入系统剪贴板（`text_pattern.cpp:1222-1248`） |
| FR-3 | 键盘快捷键支持：Ctrl+C 触发 `HandleOnCopy()`（`text_pattern.cpp:3632-3637`）；Ctrl+A 触发全选 `HandleSelectionChange(0, textSize)`（`text_pattern.cpp:3640-3648`）；Shift+方向键逐字符/逐行扩展选区（`text_pattern.cpp:3659-3680`） |
| FR-4 | `textSelectable` 存储为 `ACE_DEFINE_PROPERTY_ITEM_WITHOUT_GROUP(TextSelectableMode, TextSelectableMode, PROPERTY_UPDATE_MEASURE_SELF)`（`text_layout_property.h:213`），变更仅触发自身重测量 |
| FR-5 | 选择初始化使用词边界检测：`pManager_->GetWordBoundary(extend, start, end)` 获取 ICU 词边界（`text_pattern.cpp:344-364`）；若无词边界则回退到单字符簇选择 |
| FR-6 | 焦点模式下键盘选择：`SELECTABLE_FOCUSABLE` 模式下 Shift+Left/Right 逐字符移动选区末端，Shift+Up/Down 逐行移动（`text_pattern.cpp:3659-3680`） |
| FR-7 | 编程式选区 `selection(start, end)` 通过 `AfterLayoutTask` 延迟执行（`text_pattern.cpp:1589`），在布局完成后调用 `ActSetSelection(start, end)` 钳位并设置选区（`text_pattern.cpp:4999`） |
| FR-8 | `selectedBackgroundColor` 存储为 `ACE_DEFINE_PROPERTY_ITEM_WITHOUT_GROUP(SelectedBackgroundColor, Color, PROPERTY_UPDATE_MEASURE_SELF)`（`text_layout_property.h:211`）；传入 alpha=255 时自动降为 20% 不透明度（`js_text.cpp:459`） |
| FR-9 | `caretColor` 存储为 `CursorColor`（`text_layout_property.h:210`），实际用于选择手柄颜色（`text_select_overlay.cpp:386` 设置 `handlerColor`），Text 组件为只读无可见光标 |
| FR-10 | 拖拽初始化在 `InitCopyOption()` 中完成：当 `IsSelectableAndCopy()` 且 `host->IsDraggable()` 且 `!shiftFlag_` 时调用 `InitDragEvent()`（`text_pattern.cpp:3085`） |
| FR-11 | 拖拽数据格式区分：有 Span 子节点走 `OnDragStart()`（UDMF 含纯文本 + SpanString + 图片记录，`text_pattern.cpp:3877-3895`）；无子节点走 `OnDragStartNoChild()`（仅纯文本，`text_pattern.cpp:3960-3997`） |
| FR-12 | `selectedDragPreviewStyle` 仅含 `color` 字段（`SelectedDragPreviewStyle` 接口），存储为 `ACE_DEFINE_PROPERTY_ITEM_WITHOUT_GROUP(SelectedDragPreviewStyle, Color, PROPERTY_UPDATE_MEASURE)`（`text_layout_property.h:142`），通过 `TextDragInfo.dragBackgroundColor` 传递给拖拽预览渲染器 |
| FR-13 | `bindSelectionMenu` 以 `(TextSpanType, TextResponseType)` 为 key 存储在 `selectionMenuMap_` 中（`text_pattern.h:629`）；传 null builder 时移除对应条目 |
| FR-14 | 菜单查找使用 4 级回退：精确匹配 `(spanType, responseType)` → `(spanType, NONE)` → `(NONE, responseType)` → `(NONE, NONE)`（`text_pattern.cpp:6897-6917`） |
| FR-15 | `SelectionMenuOptions` 回调通过 `OnHandleSelectionMenuCallback` 分发，传入当前选区范围 `(start, end)`（`text_pattern.cpp:6953-6974`） |
| FR-16 | `editMenuOptions` 回调通过 `BaseTextSelectOverlay::OnUpdateOnCreateMenuCallback` 挂载到 `SelectOverlayInfo.onCreateCallback`（`base_text_select_overlay.h:251-272`） |
| FR-17 | Text 系统默认菜单项：`showCopy = true`（选区非空时）、`showCopyAll = true`（未全选时）；`showCut = false`、`showPaste = false`（只读组件）（`text_select_overlay.cpp:348-375`） |
| FR-18 | 选择手柄拖动通过 `UpdateSelectorOnHandleMove` 更新选区：第一手柄更新 `baseOffset`，第二手柄更新 `destinationOffset`（`text_select_overlay.cpp:210-222`） |
| FR-19 | 手柄拖动时每次字符索引变化触发振动反馈（`StartVibratorByIndexChange()`） |
| FR-20 | 放大镜（Magnifier）在三个场景显示：(1) 长按初始选择（`text_pattern.cpp:744-747`）；(2) 手指拖动扩展选区（`text_pattern.cpp:8156-8160`）；(3) 手柄拖动（`text_select_overlay.cpp:175-178`）。释放手指或手柄时隐藏（`RemoveMagnifierFrameNode()`） |

## 异常/豁免规则

| 编号 | 规则 |
|------|------|
| EX-1 | `selection(start, end)` 当 `start >= end` 时选区无效，不显示选中效果 |
| EX-2 | `selection(start, end)` 当索引超出 `[0, textLength]` 时自动钳位到合法范围（`text_pattern.cpp:4999`） |
| EX-3 | 选择手柄可以交叉（第一手柄越过第二手柄位置），`GetTextStart()`/`GetTextEnd()` 使用 `std::min`/`std::max` 保证选区渲染始终正确 |
| EX-4 | 文本内容为空时，长按、双击、鼠标拖选均不触发选择（各入口有 `HasContent()` 检查） |
| EX-5 | `copyOption` 传入非法枚举值时，JS Bridge 层做 range check，超出范围使用 `CopyOptions::None`（`js_text.cpp:1050`） |
| EX-6 | 容器选择场景（`selectionChild_` 非空）时，`GetOrCreateMagnifier()` 返回 nullptr，不显示放大镜（`text_pattern.cpp:9295-9308`） |

## 恢复契约

| 编号 | 规则 |
|------|------|
| RC-1 | 拖拽失败恢复：当 `DragRet != DRAG_SUCCESS` 时，选区恢复到拖拽前的 `recoverStart_`/`recoverEnd_` 范围，重新显示选择覆盖层（`text_pattern.cpp:4067-4076, 4090-4096`） |
| RC-2 | 触摸取消恢复：`DoTextSelectionTouchCancel()` 时移除放大镜、重置选区（`text_pattern.cpp:8382-8386`） |
| RC-3 | 组件卸载清理：Pattern detach 时清理放大镜和选择覆盖层（`text_pattern.cpp:127-128`） |

## 验证映射

| 编号 | 对应规格项 | 验证方式 | 验证重点 |
|------|-----------|----------|----------|
| VM-1 | AC-1.1~AC-1.6, BR-1, BR-2 | 单测 | copyOption 各枚举值下选择/复制功能开关、MARQUEE 模式强制禁用、三重门控逻辑 |
| VM-2 | AC-1.7~AC-1.9, FR-2 | 单测 + 代码审查 | 剪贴板写入三种格式（纯文本/HTML/SpanString TLV）、异步 HTML 生成 |
| VM-3 | AC-2.1~AC-2.4, BR-3 | 单测 | TextSelectableMode 三种模式下的选择能力和焦点行为 |
| VM-4 | AC-2.5~AC-2.8, FR-5, FR-6 | 单测 | 长按/双击/鼠标拖选/键盘扩展选区的触发和词边界检测 |
| VM-5 | AC-3.1~AC-3.5, FR-7 | 单测 | 编程式选区设置、钳位、守卫条件、AfterLayoutTask 延迟 |
| VM-6 | AC-4.1~AC-4.6, FR-8, FR-9 | 单测 | selectedBackgroundColor alpha 自动降低、caretColor 映射到选择手柄 |
| VM-7 | AC-5.1~AC-5.9, BR-4, FR-10~FR-12 | 单测 + 代码审查 | 拖拽优先于重选、UDMF 数据格式、拖拽预览样式 |
| VM-8 | AC-6.1~AC-6.7, FR-13~FR-15 | 单测 | bindSelectionMenu 绑定/解绑/4 级回退查找/回调 |
| VM-9 | AC-7.1~AC-7.6, BR-5, FR-16, FR-17 | 单测 + 代码审查 | editMenuOptions 定制、与 bindSelectionMenu 互补、系统菜单项可见性 |
| VM-10 | AC-8.1~AC-8.7, FR-18~FR-20, EX-3 | 单测 + 代码审查 | 手柄拖动/放大镜/振动反馈/手柄交叉 |
