...
|----|----------|-----------|----------|------|
| AC-1.1~1.8 | FR-1, BR-1, BR-2, EX-1, EX-2 | N/A（存量） | 单测 | `test/unittest/core/pattern/text/` |
| AC-2.1~2.5 | FR-2, BR-2 | N/A | 单测 | 同上 |
| AC-3.1~3.5 | FR-3, BR-3 | N/A | 单测 | 同上 |
| AC-4.1~4.8 | FR-4, BR-4, EX-3 | N/A | 单测 | 同上 |
| AC-5.1~5.5 | FR-5 | N/A | 单测 | 同上 |
| AC-6.1~6.4 | FR-6 | N/A | 单测 | 同上 |
| AC-7.1~7.4 | FR-7, BR-5 | N/A | 单测 | 同上 |
| AC-8.1~8.2 | FR-8 | N/A | 单测 | 同上 |
| AC-9.1~9.2 | FR-9 | N/A | 单测 | 同上 |
| AC-10.1~10.2 | FR-10 | N/A | 单测 | 同上 |
| AC-11.1~11.3 | FR-11 | N/A | 单测 | 同上 |

## 业务规则

| 编号 | 规则描述 | 约束条件 | 关联 AC |
|------|----------|----------|---------|
| BR-1 | textOverflow 的 ArkTS 默认值（Clip）与 C-API Reset 值（NONE）不一致 | ArkTS getter 返回 `TextOverflow::CLIP`（`text_model_ng.cpp:1319`）；C-API Reset 重置为 `TextOverflow::NONE`（`node_text_modifier.cpp:1092`）；两者在排版引擎中行为相同 | AC-1.7, AC-1.8 |
| BR-2 | textOverflow 的 Clip/Ellipsis/None 模式需要配合 maxLines 才能观察到溢出截断效果 | maxLines 默认 UINT32_MAX（无限制），无限行数下文本不会溢出 | AC-1.1, AC-1.2, AC-2.5 |
| BR-3 | minLines 与 maxLines 的钳位关系：minLines ≤ maxLines | `minLines = std::min(minLines, maxLines)`（`multiple_paragraph_layout_algorithm.cpp:1154`） | AC-3.2 |
| BR-4 | ellipsisMode 的 C++ 内部枚举名与 ArkTS API 名不一致 | C++: HEAD/MIDDLE/TAIL/MULTILINE_HEAD/MULTILINE_MIDDLE；ArkTS: START/CENTER/END/MULTILINE_START/MULTILINE_CENTER（`text_enums.h:64-70`，`utils.h:622-633`） | AC-4.1~4.5 |
| BR-5 | heightAdaptivePolicy 在 LAYOUT_CONSTRAINT_FIRST 模式下会动态减少 maxLines | 循环减少 maxLines 直到文本高度适配约束（`text_layout_algorithm.cpp:1179-1203`） | AC-7.1, AC-7.2 |
| BR-6 | enableAutoSpacing 使用 WITHOUT_GROUP 宏独立存储，不走 TextLineStyle 组传播链路 | 其他 10 项属性在 TextLineStyle 组内；enableAutoSpacing 直接存储在 TextLayoutProperty 上（`text_layout_property.h:183`） | AC-11.3 |

## 功能规则

| 编号 | 规则描述 | 触发条件 | 作用对象 | 关联 AC |
|------|----------|----------|----------|---------|
| FR-1 | `textOverflow` 设置文本溢出行为 | 设置 TextOverflow 枚举值 | `TextLayoutProperty` → TextLineStyle::propTextOverflow（`text_styles.h` TextLineStyle 组） | AC-1.1~1.8 |
| FR-2 | `maxLines` 限制文本最大行数 | 设置正整数值 | `TextLayoutProperty` → TextLineStyle::propMaxLines | AC-2.1~2.5 |
| FR-3 | `minLines` 设置文本最小行数 (@since 22) | 设置正整数值 | `TextLayoutProperty::propMinLines`（WITHOUT_GROUP，`text_layout_property.h:191`） | AC-3.1~3.5 |
| FR-4 | `ellipsisMode` 控制省略号位置 (@since 11) | 设置 EllipsisMode 枚举值，需配合 textOverflow=Ellipsis | `TextLayoutProperty` → TextLineStyle::propEllipsisMode | AC-4.1~4.8 |
| FR-5 | `wordBreak` 设置断词策略 (@since 11) | 设置 WordBreak 枚举值 | `TextLayoutProperty` → TextLineStyle::propWordBreak | AC-5.1~5.5 |
| FR-6 | `lineBreakStrategy` 设置换行质量策略 (@since 12) | 设置 LineBreakStrategy 枚举值 | `TextLayoutProperty` → TextLineStyle::propLineBreakStrategy | AC-6.1~6.4 |
| FR-7 | `heightAdaptivePolicy` 控制高度自适应策略 (@since 10) | 设置 TextHeightAdaptivePolicy 枚举值 | `TextLayoutProperty` → TextLineStyle::propHeightAdaptivePolicy | AC-7.1~7.4 |
| FR-8 | `compressLeadingPunctuation` 压缩行首标点 (@since 23) | 设置 boolean | `TextLayoutProperty` → TextLineStyle::propCompressLeadingPunctuation | AC-8.1~8.2 |
| FR-9 | `orphanCharOptimization` 孤字优化 (@since 26) | 设置 boolean | `TextLayoutProperty` → TextLineStyle::propOrphanCharOptimization | AC-9.1~9.2 |
| FR-10 | `optimizeTrailingSpace` 尾部空格优化 (@since 20) | 设置 boolean | `TextLayoutProperty` → TextLineStyle::propOptimizeTrailingSpace | AC-10.1~10.2 |
| FR-11 | `enableAutoSpacing` CJK-拉丁自动间距 (@since 20) | 设置 boolean | `TextLayoutProperty::propEnableAutoSpacing_`（WITHOUT_GROUP，`text_layout_property.h:183`） | AC-11.1~11.3 |

## 异常/豁免规则

| 编号 | 规则描述 | 触发条件 | 处理结果 | 关联 AC |
|------|----------|----------|----------|---------|
| EX-1 | textOverflow 非法值处理 | JS 层传入非数字或超出 TEXT_OVERFLOWS 数组范围 | 使用默认值 TextOverflow.NONE（`js_text.cpp:365`） | AC-1.7 |
| EX-2 | TextOverflow.MARQUEE 文本未溢出 | 文本实际宽度 ≤ 容器宽度 | 不启动跑马灯动画，文本静态显示（`text_paint_method.cpp:110`） | AC-1.6 |
| EX-3 | ellipsisMode 无前置条件 | ellipsisMode 已设置但 textOverflow ≠ Ellipsis | ellipsisMode 存储但不生效（省略号字符不注入） | AC-4.6 |
| EX-4 | maxLines 为 0 或负值 | 传入 ≤ 0 的值 | C-API 按 UINT32_MAX 处理；JS 层按 0 处理可能导致不显示文本 | AC-2.2 |
| EX-5 | minLines 非法值 | 传入负值或非数字 | 调用 ResetMinLines 清除属性（`js_text.cpp:524`） | AC-3.4 |
| EX-6 | wordBreak 超出范围 | C-API 传入超出 WORD_BREAK_TYPES 大小的值 | 回退到默认值 BREAK_WORD（index 2，`node_text_modifier.cpp:1610`） | AC-5.5 |
| EX-7 | lineBreakStrategy 非法值 | JS 层传入非数字或超出范围 | 回退到默认值 GREEDY（`js_text.cpp:408,412,417`） | AC-6.4 |

## 恢复契约

| 编号 | 触发条件 | 恢复策略 | 恢复结果 | 约束 |
|------|----------|----------|----------|------|
| RC-1 | TextOverflow.MARQUEE 跑马灯动画运行中组件失焦 | 若 MarqueeStartPolicy=ON_FOCUS，暂停动画；重新获焦后恢复（`text_content_modifier.cpp:1875-1889`） | 动画从暂停位置（或起始位置，取决于 MarqueeUpdatePolicy）继续 | MarqueeUpdatePolicy=PRESERVE_POSITION 时保持当前滚动位置，DEFAULT 时重置到起始位置 |
| RC-2 | 属性组懒初始化——TextLineStyle 属性组在首次设置任一行样式属性时通过 `GetOrCreateTextLineStyle()` 创建 | 未设置任何行样式属性时属性组为 null，各属性按主题默认值渲染 | 等同于全部使用主题默认值 | 属性组一旦创建不会被销毁，仅各 optional 字段可被 Reset |

## 验证映射

| 编号 | 对应规格项 | 验证方式 | 验证重点 |
|------|------------|----------|----------|
| VM-1 | FR-1 (textOverflow) | 单测 | Clip/Ellipsis/None 截断行为正确；MARQUEE 单行滚动 |
| VM-2 | FR-2 (maxLines) | 单测 | 行数限制生效；与 textOverflow 配合正确 |
| VM-3 | FR-3 (minLines) | 单测 | 高度扩展正确；与 maxLines 钳位关系正确 |
| VM-4 | FR-4 (ellipsisMode) | 单测 | START/CENTER/END 省略号位置正确；MULTILINE 变体多行生效 |
| VM-5 | FR-5 (wordBreak) | 单测 | NORMAL/BREAK_ALL/BREAK_WORD/HYPHENATION 断词行为正确 |
| VM-6 | FR-6 (lineBreakStrategy) | 单测 | GREEDY/HIGH_QUALITY/BALANCED 换行质量差异可观察 |
| VM-7 | FR-7 (heightAdaptivePolicy) | 单测 | 三种策略下 maxLines 和字号调整行为正确 |
| VM-8 | FR-8~FR-11 (排版优化属性) | 单测 | 各属性设置/重置/默认值行为正确 |
| VM-9 | BR-1 (默认值分歧) | 单测 | 验证 ArkTS 和 C-API 两条路径的默认值差异 |
