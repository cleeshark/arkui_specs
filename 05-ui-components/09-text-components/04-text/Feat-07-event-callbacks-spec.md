...
|-------|----------|-----------|----------|------|
| AC-1.1 | FR-1, BR-1 | TASK-7 | 单测 | HandleOnCopy test |
| AC-1.2 | FR-1, FR-2 | TASK-7 | 单测 | onWillCopy returns true test |
| AC-1.3 | FR-2, BR-2 | TASK-7 | 单测 | onWillCopy returns false test |
| AC-1.4 | FR-2, EX-1 | TASK-7 | 单测 | no onWillCopy callback test |
| AC-1.5 | FR-1 | TASK-7 | 单测 | onCopy value test |
| AC-1.6 | EX-2 | TASK-7 | 单测 | zero-width selection test |
| AC-1.7 | EX-3 | TASK-7 | 代码审查 | null guard test |
| AC-1.8 | EX-4 | TASK-7 | 单测 | empty value test |
| AC-2.1 | FR-3 | TASK-7 | 单测 | long press selection test |
| AC-2.2 | FR-3 | TASK-7 | 单测 | double click selection test |
| AC-2.3 | FR-3 | TASK-7 | 单测 | mouse drag selection test |
| AC-2.4 | FR-3 | TASK-7 | 单测 | handle drag test |
| AC-2.5 | FR-3 | TASK-7 | 单测 | select all test |
| AC-2.6 | FR-3, FR-4 | TASK-7 | 单测 | programmatic selection test |
| AC-2.7 | FR-3, FR-4 | TASK-7 | 单测 | controller setTextSelection test |
| AC-2.8 | FR-3 | TASK-7 | 单测 | keyboard selection test |
| AC-2.9 | FR-3, FR-5 | TASK-7 | 单测 | deselect test |
| AC-2.10 | FR-3 | TASK-7 | 单测 | AI entity selection test |
| AC-2.11 | FR-6 | TASK-7 | 单测 | dedup test |
| AC-2.12 | FR-3 | TASK-7 | 单测 | shift-click extend test |
| AC-2.13 | FR-3 | TASK-7 | 单测 | auto-scroll selection test |
| AC-2.14 | FR-3 | TASK-7 | 单测 | drag recover selection test |
| AC-2.15 | FR-3 | TASK-7 | 单测 | AI entity long press test |
| AC-2.16 | BR-3 | TASK-7 | 单测 | unselectable guard test |
| AC-3.1 | FR-7, BR-4 | TASK-7 | 单测 | marquee start test |
| AC-3.2 | FR-8 | TASK-7 | 单测 | marquee bounce test |
| AC-3.3 | FR-9 | TASK-7 | 单测 | marquee finish test |
| AC-3.4 | FR-9, BR-5 | TASK-7 | 单测 | loop count finish test |
| AC-3.5 | BR-5 | TASK-7 | 单测 | infinite loop test |
| AC-3.6 | FR-8, FR-9 | TASK-7 | 单测 | bounce then finish sequence test |
| AC-3.7 | FR-10, BR-6 | TASK-7 | 单测 | start side effects test |
| AC-3.8 | FR-10 | TASK-7 | 单测 | finish side effects test |
| AC-3.9 | FR-10, BR-6 | TASK-7 | 单测 | RecoverCopyOption test |
| AC-3.10 | FR-9, BR-7 | TASK-7 | 单测 | ON_FOCUS policy finish test |
| AC-3.11 | EX-5 | TASK-7 | 代码审查 | marquee not started test |
| AC-3.12 | EX-6 | TASK-7 | 单测 | stale animation id test |

## 业务规则

| 编号 | 规则 | 关联 AC |
|------|------|---------|
| BR-1 | onWillCopy → 剪贴板写入 → onCopy 的调用顺序是固定的。onWillCopy 在剪贴板操作之前调用，onCopy 在剪贴板操作之后调用。两者共享同一个 `value` 值（`GetSelectedText` 的返回值）（`text_pattern.cpp:1089-1113`） | AC-1.1, AC-1.2, AC-1.5 |
| BR-2 | onWillCopy 是唯一的复制操作取消点——返回 false 时整条流水线中断，菜单关闭但选区保持不变 | AC-1.3 |
| BR-3 | onTextSelectionChange 的触发受 `textSelectable` 和 `copyOption` 的联合控制：当 `textSelectable == UNSELECTABLE` 或 `copyOption == None` 时，`selection()` API 不生效（`text_pattern.cpp:1589` 守卫），自然不触发选区变化回调 | AC-2.16 |
| BR-4 | onMarqueeStateChange 的 START 状态具有副作用：自动关闭选择覆盖层（`CloseSelectOverlay`）并重置选区（`ResetSelection`），确保跑马灯滚动期间不显示选择 UI（`text_pattern.cpp:6996-6999`） | AC-3.7 |
| BR-5 | `marqueeOptions.loop` 控制 FINISH 触发条件：`loop > 0` 时 `marqueeCount_ >= loop` 触发 FINISH；`loop == -1`（无限循环）时永远不触发 FINISH | AC-3.4, AC-3.5 |
| BR-6 | 每次状态变化后均调用 `RecoverCopyOption()` 恢复复制能力——MARQUEE 运行期间 copyOption 被强制为 None（Feat-03 ADR-F5-1/Feat-05 AC-1.5），状态变化后需要重新评估 | AC-3.7, AC-3.9 |
| BR-7 | `MarqueeStartPolicy.ON_FOCUS` 策略下，跑马灯的启停由焦点和 hover 状态联合控制：`marqueeFocused_ || marqueeHovered_` 为 false 时 `AllowTextRace()` 返回 false → FINISH（`text_content_modifier.cpp:1868`） | AC-3.10 |

## 功能规则

| 编号 | 规则 | 关联 AC |
|------|------|---------|
| FR-1 | `onCopy` 回调在 `HandleOnCopy()` 末尾通过 `eventHub->FireOnCopy(value)` 触发（`text_pattern.cpp:1113`），参数为 `std::u16string` 类型的选中文本内容 | AC-1.1, AC-1.5 |
| FR-2 | `onWillCopy` 回调通过 `eventHub->FireOnWillCopy(value)` 触发（`text_pattern.cpp:1094`），返回 `bool`：true 允许复制，false 取消复制。未注册时 `FireOnWillCopy` 默认返回 true（`text_event_hub.h:55`） | AC-1.2, AC-1.3, AC-1.4 |
| FR-3 | `onTextSelectionChange` 通过 `TextPattern::HandleSelectionChange(start, end)` → `FireOnSelectionChange(min(start,end), max(start,end))` 触发（`text_pattern.cpp:7057`），参数自动归一化为 `(min, max)` 顺序 | AC-2.1~AC-2.15 |
| FR-4 | 编程式选区设置（`selection()` API 和 `setTextSelection()` 控制器方法）同样触发 `HandleSelectionChange`，与用户手势触发走相同路径 | AC-2.6, AC-2.7 |
| FR-5 | 取消选区使用特殊值 `(-1, -1)` 表示无选区状态——由 `HandleSelectionChange(-1, -1)` 触发，回调参数即为 `(-1, -1)`（`text_pattern.cpp:314, 1082`） | AC-2.9 |
| FR-6 | `HandleSelectionChange` 内置去重机制：如果新的 `(start, end)` 与 `textSelector_.GetStart(), textSelector_.GetEnd()` 完全相同，则直接 return 不触发回调（`text_pattern.cpp:7033-7035`） | AC-2.11 |
| FR-7 | `onMarqueeStateChange(START)` 在 `ResumeTextRace(bounce=false)` 中触发（`text_content_modifier.cpp:1765`），此时 `marqueeCount_` 被重置为 0 | AC-3.1 |
| FR-8 | `onMarqueeStateChange(BOUNCE)` 在动画完成回调中、当 `racePercent == marqueeRaceMaxPercent_` 时触发（`text_content_modifier.cpp:1821`），随后 `marqueeCount_++` | AC-3.2, AC-3.6 |
| FR-9 | `onMarqueeStateChange(FINISH)` 在动画完成回调中、当 `!AllowTextRace()` 时触发（`text_content_modifier.cpp:1824-1825`）；`AllowTextRace()` 检查三个条件：marqueeSet_ && start、loop 计数、ON_FOCUS 策略（`text_content_modifier.cpp:1860-1872`） | AC-3.3, AC-3.4, AC-3.10 |
| FR-10 | `FireOnMarqueeStateChange` 执行后根据状态执行副作用（`text_pattern.cpp:6996-7004`）：START → CloseSelectOverlay + ResetSelection + isMarqueeRunning_=true；FINISH → isMarqueeRunning_=false；所有状态 → RecoverCopyOption() | AC-3.7, AC-3.8, AC-3.9 |

## 异常/豁免规则

| 编号 | 规则 | 关联 AC |
|------|------|---------|
| EX-1 | 未注册 onWillCopy 回调时，`FireOnWillCopy` 默认返回 true，复制流程正常进行 | AC-1.4 |
| EX-2 | 选区有效但 `start == end`（零宽选区）时，`HandleOnCopy` 不触发复制回调，而是调用 `HandleSelectionChange(-1, -1)` 清除选区 | AC-1.6 |
| EX-3 | `clipboard_` 为 null 或 `GetDataDetectorAdapter()` 返回 null 时，`HandleOnCopy` 通过 `CHECK_NULL_VOID` 立即返回，不触发任何回调 | AC-1.7 |
| EX-4 | 选中文本为空字符串时，`onWillCopy` 不被调用（`value.empty()` 守卫）；后续 `FireOnCopy` 被 `CHECK_NULL_VOID(!value.empty())` 阻止 | AC-1.8 |
| EX-5 | `marqueeSet_ == false` 或 `marqueeOption_.start == false` 时，`AllowTextRace()` 返回 false，`ResumeTextRace` 在入口处直接返回，不触发任何状态回调 | AC-3.11 |
| EX-6 | 动画被替代（新动画启动导致 `marqueeAnimationId` 不匹配）时，旧动画的完成回调检测到 ID 不匹配后直接返回，不触发 BOUNCE/FINISH | AC-3.12 |

## 恢复契约

| 编号 | 规则 | 关联 AC |
|------|------|---------|
| RC-1 | onWillCopy 返回 false 取消复制后，选区状态不变，菜单关闭。用户可重新打开菜单再次尝试复制 | AC-1.3 |
| RC-2 | 跑马灯 FINISH 后，`isMarqueeRunning_` 置 false，`RecoverCopyOption()` 恢复复制能力。如果 `marqueeOption_.start` 仍为 true 且满足条件，可重新启动 | AC-3.8, AC-3.9 |
| RC-3 | 拖拽失败时通过 `HandleSelectionChange(recoverStart_, recoverEnd_)` 恢复之前的选区，触发 onTextSelectionChange 回调 | AC-2.14 |

## 验证映射

| 编号 | 对应规格项 | 验证方式 | 验证重点 |
|------|-----------|----------|----------|
| VM-1 | AC-1.1~AC-1.8, FR-1, FR-2, BR-1, BR-2 | 单测 | onWillCopy/onCopy 调用顺序、参数值、拦截能力、边界条件 |
| VM-2 | AC-2.1~AC-2.16, FR-3~FR-6, BR-3 | 单测 | onTextSelectionChange 全场景触发、去重机制、特殊值语义、门控条件 |
| VM-3 | AC-3.1~AC-3.12, FR-7~FR-10, BR-4~BR-7 | 单测 | 跑马灯三状态触发时机、副作用、loop 控制、ON_FOCUS 策略、动画 ID 防护 |
| VM-4 | EX-1~EX-6 | 单测 + 代码审查 | 空回调默认行为、空值守卫、marquee 未启动守卫 |
| VM-5 | RC-1~RC-3 | 单测 | 拦截后恢复、FINISH 后恢复、拖拽失败恢复 |
