...
|----|----------|-----------|----------|------|
| AC-1.1~1.5 | FR-1, BR-2 | N/A（存量） | 单测 | `test/unittest/core/pattern/text/` |
| AC-2.1~2.2 | FR-2 | N/A | 单测 | 同上 |
| AC-3.1~3.7 | FR-3, FR-4, BR-3 | N/A | 单测 | 同上 |
| AC-4.1~4.4 | FR-5, EX-1, BR-1 | N/A | 单测 | 同上 |
| AC-5.1~5.2 | FR-6 | N/A | 单测 | 同上 |
| AC-6.1~6.3 | FR-7 | N/A | 单测 | 同上 |
| AC-7.1~7.4 | FR-8 | N/A | 单测 | 同上 |
| AC-8.1~8.3 | FR-9 | N/A | 单测 | 同上 |
| AC-9.1~9.8 | FR-10~FR-14, BR-2, EX-3 | N/A | 单测 | 同上 |
| AC-10.1~10.6 | FR-15, FR-16 | N/A | 单测 | 同上 |

## 业务规则

| 编号 | 规则描述 | 约束条件 | 关联 AC |
|------|----------|----------|---------|
| BR-1 | 卡片场景仅支持 `HarmonyOS Sans` 字族 | form 场景（`text.d.ts` @form 标注） | AC-4.4 |
| BR-2 | 自适应字号与固定字号互斥：自适应生效时 fontSize 不生效 | 需 minFontSize + maxFontSize + (maxLines 或布局约束) 三方配合 | AC-1.5, AC-9.7 |
| BR-3 | enableVariableFontWeight 双路径：ArkTS 通过 FontSettingOptions 传入，C API 使用独立的 NODE_IMMUTABLE_FONT_WEIGHT 属性 | ArkTS 存入 EnableVariableFontWeight + VariableFontWeight；C API 仅存入 VariableFontWeight | AC-3.6, AC-3.7 |

## 功能规则

| 编号 | 规则描述 | 触发条件 | 作用对象 | 关联 AC |
|------|----------|----------|----------|---------|
| FR-1 | `fontSize` 设置文本字号，单位 fp | 设置有效正数值（number/string/Resource） | `TextLayoutProperty` → FontStyle::propFontSize (`text_styles.h:239`) | AC-1.1~1.4 |
| FR-2 | `fontColor` 设置文本颜色 | 设置合法 ResourceColor | `TextLayoutProperty` → FontStyle::propTextColor (`text_styles.h:240`) | AC-2.1~2.2 |
| FR-3 | `fontWeight` 设置文本字重 | 设置 100~900 数值、FontWeight 枚举或合法字符串 | `TextLayoutProperty` → FontStyle::propFontWeight (`text_styles.h:243`) | AC-3.1~3.5 |
| FR-4 | `fontWeight` + FontSettingOptions 启用可变字重 | options.enableVariableFontWeight = true | `TextLayoutProperty` → FontStyle::propEnableVariableFontWeight + propVariableFontWeight (`text_styles.h:245-246`) | AC-3.6~3.7 |
| FR-5 | `fontFamily` 设置字族 | 设置字族名称字符串或 Resource | `TextLayoutProperty` → FontStyle::propFontFamily (`text_styles.h:248`) | AC-4.1~4.4 |
| FR-6 | `fontStyle` 设置斜体 | 设置 FontStyle 枚举 | `TextLayoutProperty` → FontStyle::propItalicFontStyle (`text_styles.h:242`) | AC-5.1~5.2 |
| FR-7 | `font()` 复合设置，内部分解为独立 setter 调用 | 传入 Font 对象（`text_model_ng.cpp:118`） | FontSize + FontWeight + FontFamily + ItalicFontStyle | AC-6.1~6.3 |
| FR-8 | `fontFeature` 设置 OpenType 特性 (@since 12) | 传入特性标签字符串 | `TextLayoutProperty` → FontStyle::propFontFeature (`text_styles.h:249`) | AC-7.1~7.4 |
| FR-9 | `fontVariations` 设置可变字体轴 (@since 26.0.0) | 传入 FontVariation 数组 | `TextLayoutProperty` → FontStyle::propFontVariations (`text_styles.h:250`) | AC-8.1~8.3 |
| FR-10 | `minFontSize` 设置自适应最小字号 | 正数值 | `TextLayoutProperty` → FontStyle::propAdaptMinFontSize (`text_styles.h:254`) | AC-9.1 |
| FR-11 | `maxFontSize` 设置自适应最大字号 | 正数值 | `TextLayoutProperty` → FontStyle::propAdaptMaxFontSize (`text_styles.h:255`) | AC-9.1 |
| FR-12 | `heightAdaptivePolicy` 选择自适应策略 | MAX_LINES_FIRST / MIN_FONT_SIZE_FIRST / LAYOUT_CONSTRAINT_FIRST | `TextLayoutProperty` → TextLineStyle::propHeightAdaptivePolicy (`text_styles.h:378`) | AC-9.4~9.6 |
| FR-13 | 自适应字号无效值处理 | minFontSize 或 maxFontSize ≤ 0 | 回退至 fontSize | AC-9.3 |
| FR-14 | 自适应字号三方配合要求 | 需 minFontSize + maxFontSize + (maxLines 或布局约束) 同时具备 | 缺少任一条件自适应不生效 | AC-9.2 |
| FR-15 | `minFontScale` 限制缩放下限 (@since 12) | 值域 [0, 1]，< 0 按 0，> 1 按 1 | `TextLayoutProperty` → FontStyle::propMinFontScale | AC-10.1, AC-10.3~10.4 |
| FR-16 | `maxFontScale` 限制缩放上限 (@since 12) | 值域 [1, +∞)，< 1 按 1 | `TextLayoutProperty` → FontStyle::propMaxFontScale | AC-10.2, AC-10.5 |

## 异常/豁免规则

| 编号 | 规则描述 | 触发条件 | 处理结果 | 关联 AC |
|------|----------|----------|----------|---------|
| EX-1 | 无效字族回退 | fontFamily 指定的字族不存在 | 回退到系统默认字族（静默，无错误提示） | AC-4.2 |
| EX-2 | 无效 fontWeight 字符串 | 传入非法字符串（非数字、非枚举名） | 使用默认值 FontWeight.Normal (400) | AC-3.4 |
| EX-3 | 自适应字号缺少配合属性 | 仅设置 minFontSize 未设置 maxFontSize 或 maxLines/布局约束 | 自适应不生效，静默回退到 fontSize（无错误提示） | AC-9.2 |
| EX-4 | 不支持的可变字体轴 | fontVariations 指定了当前字体不支持的轴标签 | 该轴设置被忽略，不影响其他轴 | AC-8.2 |
| EX-5 | minFontScale/maxFontScale 异常值 | 非数字输入 | 设置不生效，保持默认行为 | AC-10.6 |

## 恢复契约

| 编号 | 触发条件 | 恢复策略 | 恢复结果 | 约束 |
|------|----------|----------|----------|------|
| RC-1 | 属性组懒初始化——FontStyle 属性组在首次设置任一字体属性时通过 `GetOrCreateFontStyle()` 创建 | 未设置任何字体属性时属性组为 null，各属性按主题默认值渲染 | 等同于全部使用主题默认值 | 属性组一旦创建不会被销毁，仅各 optional 字段可被 Reset |

## 验证映射

| 编号 | 对应规格项 | 验证方式 | 验证重点 |
|------|------------|----------|----------|
| VM-1 | FR-1~FR-6 (核心字体属性) | 单测 | 各属性设置后渲染结果与设置值一致 |
| VM-2 | FR-7 (font 复合) | 单测 | 未指定字段不覆盖已有值 |
| VM-3 | FR-8~FR-9 (fontFeature/fontVariations) | 单测 | OpenType 特性生效、不支持的轴被忽略 |
| VM-4 | FR-10~FR-14 (自适应字号) | 单测 | 三方配合条件、策略优先级、无效值回退 |
| VM-5 | FR-15~FR-16 (fontScale) | 单测 | 系统缩放越界时夹紧到范围边界 |
| VM-6 | BR-3 (enableVariableFontWeight 双路径) | 单测 | ArkTS 和 C API 两条路径均可启用可变字重 |
