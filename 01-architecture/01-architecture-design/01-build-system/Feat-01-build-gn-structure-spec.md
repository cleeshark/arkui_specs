...
本特性补录 ace_engine 现有 BUILD.gn/gni 构建结构，包括平台发现、全局 config、主库聚合、框架 source_set、前端生成物、接口包、扩展组件和测试入口。本文档只描述当前实现，不提出构建行为修改。
...
|----------|------|----------|---------|
| 无 | N/A | 本 Feature 不新增 ArkTS、C API、NAPI 或 ABI。 | AC-1.1 至 AC-4.4 |

### 变更/废弃 API

| API 名称 | 变更类型 | 关联 AC |
|----------|----------|---------|
| 无 | N/A | AC-1.1 至 AC-4.4 |
...
| 不修改公共 API 或依赖 | 本补录不新增 BUILD.gn deps、不修改接口签名、不手工编辑 generated 文件。 | AC-1.1 至 AC-4.4 |

## 非功能性需求

| 类型 | 指标/阈值 | 验证方式 | 证据 |
|------|-----------|----------|------|
| 性能 | 不引入新增构建步骤，不改变现有增量构建输入输出。 | 文档差异审查。 | 本 Feature 只新增 specs 文档和索引。 |
| 内存 | 不改变编译器、链接器和生成脚本内存占用。 | 文档差异审查。 | 无 BUILD.gn 变更。 |
| 安全 | 不新增外部依赖、权限或运行时入口。 | 文档差异审查。 | API 变更分析为空。 |
| 可靠性 | 所有构建结构结论可由 file:line 追溯。 | 源码审查。 | 本文 AC、FR、EX、RC 引用现有源码行。 |
| 问题定位 | 主库、前端生成物、接口包、测试入口均给出定位路径。 | 源码审查。 | US-1 至 US-4。 |
...
| 版本升级 | 是 | 记录当前构建结构，帮助后续升级保持 target 和 bundle 入口兼容。 | AC-2.1 至 AC-4.4 |
| 生态兼容 | 是 | NDK/NAPI/ANI 和 bundle 入口不变，补录有助于发布产物核对。 | AC-4.1, AC-4.4 |
