...
| AC-1.2 | WHEN AceContainer::Initialize THEN 仅条件性调用 InitializeFrontend（DECLARATIVE_JS/STATIC_HYBRID_DYNAMIC/DYNAMIC_HYBRID_STATIC 类型跳过前端初始化）；管线/TaskExecutor/AssetManager 在 UIContentImpl::CommonInitialize 中分别初始化，不在 Initialize() 中 | 正常 |
| AC-1.3 | WHEN AceContainer::Destroy THEN 管线先销毁（异步 PostTask 到 UI 线程），前端后销毁（异步 PostTask 到 JS 线程）；子容器只销毁管线并提前返回；View 在 DestroyView() 中独立销毁 | 正常 |
...
| AC-1.11 | WHEN ContainerType 枚举 THEN 非连续赋值：STAGE=1/FA=2/PA_SERVICE=3/PA_DATA=4/PA_FORM=5/FA_SUBWINDOW=6/DC=7/WINDOW_FREE=9(值8缺失)/COMPONENT_SUBWINDOW=10/PLUGIN_SUBCONTAINER=20，ID 范围分区每种 100000 个 ID | 边界 |
| AC-1.12 | WHEN AceContainer::Dispatch THEN 异步派发 JS 消息；WHEN DispatchSync THEN 空函数体 no-op（不执行任何分发逻辑，resData/position 输出为零值） | 正常 |
...
作为一个 ACE 引擎开发者，我希望 PlatformWindow 抽象基类定义 VSync 和根节点接口，PlatformWindow::Create 无条件返回 Platform::RSWindow，实际渲染后端选择在 AceContainer::SetView 中通过 ENABLE_ROSEN_BACKEND 宏决定。

| AC编号 | 验收标准 | 类型 |
|--------|----------|------|
| AC-3.1 | WHEN PlatformWindow::Create(AceView*) THEN 无条件返回 Platform::RSWindow（无构建目标条件分支）；实际渲染后端选择在 AceContainer::SetView 中通过 ENABLE_ROSEN_BACKEND 编译宏决定：启用时直接创建 NG::RosenWindow（跳过 PlatformWindow::Create），未启用时走 PlatformWindow::Create → Platform::RSWindow 旧管线路径 | 正常 |
...
| AC-3.5 | WHEN JsMessageDispatcher::Dispatch THEN 异步派发 JS 消息；WHEN DispatchSync THEN 空函数体 no-op（不执行任何分发逻辑，与 AC-1.12 一致） | 正常 |
...
| AC编号 | 验收标准 | 类型 |
|--------|----------|------|
| AC-5.1 | WHEN UIContentImpl::Initialize THEN 委托 InitializeInner→CommonInitialize（~675 行初始化编排链），依次完成：设备属性/API 版本/新管线判定/AssetManager 创建/Container 创建与配置/View 创建与后端选择(SetView)/窗口监听注册/安全区初始化/LocalStorage 配置 等 30+ 步骤；AceContainer::Initialize() 仅是其中一步条件性调用 | 正常 |
...
|----|----------|-----------|----------|------|
| AC-1.1~1.12 | R-1~12 | TASK-F01-01 | 单测 | container.h, ace_container.h |
| AC-2.1~2.5 | R-13~17 | TASK-F01-02 | 单测 | ace_application_info_impl.h |
| AC-3.1~3.5 | R-18~22 | TASK-F01-03 | 单测 | platform_window.h |
| AC-4.1~4.8 | R-23~30 | TASK-F01-04 | 编译检查 | ace_config.gni |
| AC-5.1~5.5 | R-31~35 | TASK-F01-05 | 单测 | ui_content_impl.h |
...
| R-2 | 行为 | Initialize | 仅条件性调用 InitializeFrontend；DECLARATIVE_JS 等类型跳过；管线/TaskExecutor/AssetManager 在 CommonInitialize 中初始化 | CommonInitialize 委托链 | AC-1.2 |
| R-3 | 行为 | Destroy | 管线→前端（均异步 PostTask），子容器仅销毁管线 | 异步派发，非同步顺序 | AC-1.3 |
...
| R-11 | 边界 | ContainerType 枚举 | 非连续赋值，值8缺失，PLUGIN=20 | STAGE=1,FA=2,...,DC=7,WINDOW_FREE=9,COMPONENT_SUBWINDOW=10,PLUGIN=20 | AC-1.11 |
| R-12 | 行为 | Dispatch/DispatchSync | Dispatch 异步派发；DispatchSync 空函数体 no-op | DispatchSync 不执行任何逻辑 | AC-1.12 |
...
| R-18 | 行为 | PlatformWindow::Create | 无条件返回 Platform::RSWindow；后端选择在 AceContainer::SetView ENABLE_ROSEN_BACKEND 宏 | OHOS 生产构建走 NG::RosenWindow，Create 不被调用 | AC-3.1 |
...
| R-22 | 行为 | JsMessageDispatcher::Dispatch/DispatchSync | Dispatch 异步派发；DispatchSync 空函数体 no-op | AceContainer 实现 Dispatch；DispatchSync 为空 no-op | AC-3.5 |
...
| R-31 | 行为 | UIContentImpl::Initialize | 委托 CommonInitialize ~675 行编排链（30+ 步骤） | 仅"创建 AceContainer 并初始化"严重低估复杂度 | AC-5.1 |
...
| 开放范围 | InnerApi |
| 错误码 | N/A |
| 行为说明 | 仅条件性调用 InitializeFrontend()；DECLARATIVE_JS/DECLARATIVE_CJ/STATIC_HYBRID_DYNAMIC/DYNAMIC_HYBRID_STATIC 类型跳过前端初始化，函数体为空。管线/TaskExecutor/AssetManager 在 UIContentImpl::CommonInitialize 中分别初始化 |
| 关联 AC | AC-1.2 |
...
| 属性 | 值 |
|------|-----|
| 函数签名 | `UIContentErrorCode UIContentImpl::Initialize(OHOS::Rosen::Window* window, const std::string& url, napi_value storage)` |
| 返回值 | `UIContentErrorCode` |
| 开放范围 | InnerApi |
| 错误码 | 返回 UIContentErrorCode 枚举 |
| 行为说明 | 委托 InitializeInner→CommonInitialize（~675 行编排链），依次完成 30+ 初始化步骤：设备属性、API 版本判定、AssetManager 创建、Container 创建与配置、View 创建与后端选择、窗口监听、安全区、LocalStorage 等。AceContainer::Initialize() 仅是其中一步条件性调用 |
| 关联 AC | AC-5.1 |
...
context-queries:
  - repo: "openharmony/ace_engine"
    query: "Container+JsMessageDispatcher 双继承模式和 AceContainer/DialogContainer/PaContainer 三种容器子类"
  - repo: "openharmony/ace_engine"
