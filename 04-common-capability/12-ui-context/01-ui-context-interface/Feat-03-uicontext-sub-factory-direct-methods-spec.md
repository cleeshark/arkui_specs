...
**静态前端实现**: @ohos.arkui.UIContext.ts 中所有 Impl 类方法均遵循 Sync_InstanceId/Restore_InstanceId 模式 [ANI桥接: ArkUIAniModule._Common_Sync_InstanceId / _Common_Restore_InstanceId]
**动态前端实现**: jsUIContext.js 中所有子对象方法通过 withInstanceId(instanceId_) 包装 NAPI 调用，实现等效实例路由守卫
...
**静态前端实现**: @ohos.arkui.UIContext.ts 所有 Impl 类构造函数接受 instanceId 参数 [ANI桥接: 构造时赋值 this.instanceId_]
**动态前端实现**: jsUIContext.js 工厂方法通过 new X(instanceId_) + withInstanceId 创建子对象实例 (getRouter:500, getPromptAction:512, getOverlayManager:606, getComponentUtils:599, getComponentSnapshot:554, getFocusController:889, getDragController:490, getCursorController:924, getSmartGestureController:1125, getUIObserver:779, getUIInspector:529, getMediaQuery:524, getFont:495, getMeasureUtils:803)
...
**静态前端实现**: @ohos.arkui.UIContext.ts PromptActionImpl.openCustomDialog [ANI桥接: ArkUIAniModule._PromptAction_openCustomDialog]
**动态前端实现**: jsUIContext.js:512-515 getPromptAction → new PromptAction(instanceId_) → openCustomDialog: withInstanceId → getOverlayManager().openCustomDialog1(contentPtr, options, optionsInternal)
...
**静态前端实现**: @ohos.arkui.UIContext.ts RouterImpl.pushUrl/replaceUrl/back → ArkUIAniModule._Router_* → PageRouterManager; UIObserverImpl.on('routerPageUpdate') → ArkUIAniModule._UIObserver_on_routerPageUpdate
**动态前端实现**: jsUIContext.js Router:1373-1516 → withInstanceId → globalThis.requireNapi('router'); UIObserver:256-321 → on('routerPageUpdate') 通过 PipelineContext 事件分发
...
**静态前端实现**: @ohos.arkui.UIContext.ts animateTo/animateToImmediately/keyframeAnimateTo [ANI桥接: ArkUIAniModule 动画闭包派发]
**动态前端实现**: jsUIContext.js animateTo:626 (withInstanceId → Context.animateTo), animateToImmediately:797 (withInstanceId → Context.animateToImmediately), keyframeAnimateTo:791 (withInstanceId → Context.keyframeAnimateTo), createAnimator:505 (withInstanceId + NAPI requireNapi('animator'))
...
**静态前端实现**: @ohos.arkui.UIContext.ts DetachedRootEntryManager [ANI桥接: createUiDetachedRoot/destroyUiDetachedRoot]
**动态前端实现**: jsUIContext.js createUiDetachedRoot/destroyUiDetachedRoot (withInstanceId → PeerNode创建/销毁)
...
**静态前端实现**: @ohos.arkui.UIContext.ts OverlayManagerImpl.addComponentContent, PromptActionImpl.openCustomDialog [ANI桥接: KPointer 提取 + ArkUIAniModule 调用]
**动态前端实现**: jsUIContext.js getOverlayManager:606 (lazy new OverlayManager(instanceId_) + withInstanceId + setOverlayManagerOptions), getPromptAction:512 (new PromptAction(instanceId_) + withInstanceId)
...
**静态前端实现**: @ohos.arkui.UIContext.ts setKeyboardAvoidMode/getKeyboardAvoidMode [ANI桥接: ArkUIAniModule._Common_setKeyboardAvoidMode / _Common_getKeyboardAvoidMode]
**动态前端实现**: jsUIContext.js setKeyboardAvoidMode (withInstanceId → Context.setKeyboardAvoidMode), getKeyboardAvoidMode (withInstanceId → Context.getKeyboardAvoidMode)
...
**静态前端实现**: @ohos.arkui.UIContext.ts setOverlayManagerOptions [ANI桥接: ArkUIAniModule._OverlayManager_setOverlayManagerOptions]
**动态前端实现**: jsUIContext.js setOverlayManagerOptions (withInstanceId → Context.setOverlayManagerOptions)
...
**静态前端实现**: @ohos.arkui.UIContext.ts:1183-1194 [ANI桥接: ArkUIAniModule._Common_freezeUINodeById]
**动态前端实现**: jsUIContext.js:1051-1058 (withInstanceId → getUINativeModule().common.freezeUINodeById)
...
**静态前端实现**: @ohos.arkui.UIContext.ts TargetInfo(id: string|number, componentId?: number) [用于 PromptActionImpl/OverlayManagerImpl 绑定目标节点]
**动态前端实现**: jsUIContext.js 同样使用 TargetInfo 对象传递目标节点信息（openPopup/openMenu 等方法）
...
**静态前端实现**: @ohos.arkui.UIContext.ts SmartGestureControllerImpl [ANI桥接: ArkUIAniModule._SmartGestureController_*]
**动态前端实现**: jsUIContext.js:1125-1130 getSmartGestureController → null-check惰性 new SmartGestureController(instanceId_) + withInstanceId
...
**静态前端实现**: @ohos.arkui.UIContext.ts ComponentSnapshotImpl.createFromBuilder [ANI桥接: ArkUIAniModule._ComponentSnapshot_createFromBuilder]
**动态前端实现**: jsUIContext.js:554-557 getComponentSnapshot → new ComponentSnapshot(instanceId_) + withInstanceId → NAPI模块调用
...
**静态前端实现**: @ohos.arkui.UIContext.ts:1224-1234 (getWindowWidthBreakpoint), :1235-1245 (getWindowHeightBreakpoint) [ANI桥接: ArkUIAniModule._Common_getWindowWidthBreakpoint / _Common_getWindowHeightBreakpoint]
**动态前端实现**: jsUIContext.js getWindowWidthBreakpoint:950 (withInstanceId → getUINativeModule().common.getWindowWidthBreakpoint), getWindowHeightBreakpoint:956 (withInstanceId → getUINativeModule().common.getWindowHeightBreakpoint)
...
**静态前端实现**: @ohos.arkui.UIContext.ts:1324-1349 [ANI桥接: ArkUIAniModule._GetPageRootNode + isAvailable 校验]
**动态前端实现**: jsUIContext.js:1104-1117 (isAvailable 校验 + withInstanceId → getUINativeModule().getPageRootNode)
...
**静态前端实现**: @ohos.arkui.UIContext.ts:1496-1498 [ANI桥接: ArkUIAniModule._Common_IsEasySplit → common_ani_modifier.cpp:120-128]
**动态前端实现**: jsUIContext.js:1119-1123 (withInstanceId → Context.isEasySplit)
...
**静态前端实现**: 静态方法，无实例绑定，直接调用 getUINativeModule().resource.setResourceManagerCacheMaxCountForHSP [ANI桥接: resource 模块原生调用]
**动态前端实现**: jsUIContext.js:486-488 (静态方法，无 withInstanceId，getUINativeModule().resource.setResourceManagerCacheMaxCountForHSP)
...
**静态前端实现**: @ohos.arkui.UIContext.ts:1307-1318 [ANI桥接: ArkUIAniModule._CustomNode_QueryNavDestinationInfo1 / _CustomNode_QueryRouterPageInfo1]
**动态前端实现**: jsUIContext.js:877-880 (withInstanceId → getUINativeModule().getPageInfoByUniqueId)
...
**静态前端实现**: @ohos.arkui.UIContext.ts isAvailable() → UIContextUtil.availableInstanceIds_.has(this.instanceId_) [ANI桥接: ArkUIAniModule._Common_isAvailable]
**动态前端实现**: jsUIContext.js:1061-1063 isAvailable() → __availableInstanceIds__.has(this.instanceId_)
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts:866 getRouter() → new RouterImpl(this.instanceId_) [ANI桥接: NAPI requireNapi('router')]
**动态前端实现**: jsUIContext.js:500-503 getRouter() → new Router(this.instanceId_) + 每个方法 withInstanceId → globalThis.requireNapi('router')
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getPromptAction() → new PromptActionImpl(this.instanceId_) [ANI桥接: ArkUIAniModule._PromptAction_*]
**动态前端实现**: jsUIContext.js:512-515 getPromptAction() → new PromptAction(this.instanceId_) + 每个方法 withInstanceId → NAPI模块调用
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getOverlayManager() → new OverlayManagerImpl(this.instanceId_) [ANI桥接: ArkUIAniModule._OverlayManager_*]
**动态前端实现**: jsUIContext.js:606-612 getOverlayManager() → new OverlayManager(this.instanceId_) + setOverlayManagerOptions() + withInstanceId → NAPI模块调用
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getComponentUtils() → new ComponentUtilsImpl(this.instanceId_) [ANI桥接: ArkUIAniModule._ComponentUtils_*]
**动态前端实现**: jsUIContext.js:599-604 getComponentUtils() → null-check惰性 new ComponentUtils(this.instanceId_) + withInstanceId → getUINativeModule().componentUtils
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getComponentSnapshot() → new ComponentSnapshotImpl(this.instanceId_) [ANI桥接: ArkUIAniModule._ComponentSnapshot_*]
**动态前端实现**: jsUIContext.js:554-557 getComponentSnapshot() → new ComponentSnapshot(this.instanceId_) + withInstanceId → NAPI模块调用
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getFocusController() → new FocusControllerImpl(this.instanceId_) [ANI桥接: ArkUIAniModule._FocusController_*]
**动态前端实现**: jsUIContext.js:889-894 getFocusController() → null-check惰性 new FocusController(this.instanceId_) + withInstanceId → NAPI模块调用
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getDragController() → new DragControllerImpl(this.instanceId_) [ANI桥接: ArkUIAniModule._DragController_*]
**动态前端实现**: jsUIContext.js:490-493 getDragController() → new DragController(this.instanceId_) + withInstanceId → NAPI模块调用
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getUIObserver() → new UIObserverImpl(this.instanceId_) [ANI桥接: ArkUIAniModule._UIObserver_*]
**动态前端实现**: jsUIContext.js:779-782 getUIObserver() → new UIObserver(this.instanceId_) + withInstanceId → NAPI模块调用
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getUIInspector() → new UIInspectorImpl(this.instanceId_) [ANI桥接: ArkUIAniModule._UIInspector_*]
**动态前端实现**: jsUIContext.js:529-532 getUIInspector() → new UIInspector(this.instanceId_) + withInstanceId → NAPI模块调用
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getMediaQuery() → new MediaQueryImpl(this.instanceId_) [ANI桥接: NAPI requireNapi('mediaquery')]
**动态前端实现**: jsUIContext.js:524-527 getMediaQuery() → new MediaQuery(this.instanceId_) + matchMediaSync → NAPI requireNapi('mediaquery')
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getFont() → new FontImpl(this.instanceId_) [ANI桥接: ArkUIAniModule._Font_* + Sync/Restore守卫]
**动态前端实现**: jsUIContext.js:495-498 getFont() → new Font(this.instanceId_) + 每个方法 withInstanceId → globalThis.requireNapi('font')
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getMeasureUtils() → new MeasureUtilsImpl(this.instanceId_) [ANI桥接: ArkUIAniModule._MeasureUtils_*]
**动态前端实现**: jsUIContext.js:803-806 getMeasureUtils() → new MeasureUtils(this.instanceId_) + withInstanceId → NAPI模块调用
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getCursorController() → new CursorControllerImpl(this.instanceId_) [ANI桥接: ArkUIAniModule._CursorController_*]
**动态前端实现**: jsUIContext.js:924-929 getCursorController() → null-check惰性 new CursorController(this.instanceId_) + withInstanceId → NAPI模块调用
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getSmartGestureController() → new SmartGestureControllerImpl(this.instanceId_) [ANI桥接: ArkUIAniModule._SmartGestureController_*]
**动态前端实现**: jsUIContext.js:1125-1130 getSmartGestureController() → null-check惰性 new SmartGestureController(this.instanceId_) + withInstanceId → NAPI模块调用
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getContextMenuController() → new ContextMenuControllerImpl(this.instanceId_) [ANI桥接: ArkUIAniModule._ContextMenuController_*]
**动态前端实现**: jsUIContext.js:931-936 getContextMenuController() → null-check惰性 new ContextMenuController(this.instanceId_) + withInstanceId → NAPI模块调用
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getTextMenuController() → new TextMenuControllerImpl(this.instanceId_) [ANI桥接: ArkUIAniModule._TextMenuController_*]
**动态前端实现**: jsUIContext.js:1044-1049 getTextMenuController() → null-check惰性 new TextMenuController(this.instanceId_) + withInstanceId → NAPI模块调用
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getMagnifier() → new MagnifierImpl(this.instanceId_) [ANI桥接: ArkUIAniModule._Magnifier_*]
**动态前端实现**: jsUIContext.js:1079-1084 getMagnifier() → null-check惰性 new Magnifier(this.instanceId_) + withInstanceId → NAPI模块调用
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getAtomicServiceBar() → new AtomicServiceBarInternal(this.instanceId_) [ANI桥接: ArkUIAniModule._AtomicServiceBar_*]
**动态前端实现**: jsUIContext.js:765-777 getAtomicServiceBar() → conditional new AtomicServiceBar(this.instanceId_) + withInstanceId → NAPI模块调用
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts getLuminanceSampler(target) → new LuminanceSamplerImpl(this.instanceId_, target) [ANI桥接: ArkUIAniModule._LuminanceSampler_*]
**动态前端实现**: jsUIContext.js 无独立 getLuminanceSampler 方法（推测仅静态前端提供）
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts setOverlayManagerOptions/getOverlayManagerOptions [ANI桥接: ArkUIAniModule._OverlayManager_*]
**动态前端实现**: jsUIContext.js setOverlayManagerOptions (withInstanceId → Context.setOverlayManagerOptions), getOverlayManagerOptions (withInstanceId → Context.getOverlayManagerOptions)
...
```
**静态前端实现**: @ohos.arkui.UIContext.ts TargetInfo(id: string|number, componentId?: number) [用于 PromptActionImpl/OverlayManagerImpl 绑定目标节点]
**动态前端实现**: jsUIContext.js 同样使用 TargetInfo 对象传递目标节点信息（openPopup/openMenu/openBindSheet 等方法）
...
**静态前端实现**: @ohos.arkui.UIContext.ts:1224-1234 [ANI桥接: ArkUIAniModule._Common_getWindowWidthBreakpoint]
**动态前端实现**: jsUIContext.js:950-954 (withInstanceId → getUINativeModule().common.getWindowWidthBreakpoint)
...
**静态前端实现**: @ohos.arkui.UIContext.ts:1235-1245 [ANI桥接: ArkUIAniModule._Common_getWindowHeightBreakpoint]
**动态前端实现**: jsUIContext.js:956-960 (withInstanceId → getUINativeModule().common.getWindowHeightBreakpoint)
...
**静态前端实现**: @ohos.arkui.UIContext.ts:1925-1928 [ANI桥接: 合成 NavDestinationInfo + RouterPageInfo]
**动态前端实现**: jsUIContext.js:877-880 (withInstanceId → getUINativeModule().getPageInfoByUniqueId)
...
**静态前端实现**: @ohos.arkui.UIContext.ts getWindowWidthBreakpoint:1224-1234 [ANI桥接: ArkUIAniModule._Common_getWindowWidthBreakpoint], getWindowHeightBreakpoint:1235-1245 [ANI桥接: ArkUIAniModule._Common_getWindowHeightBreakpoint], getPageRootNode:1324-1349 [ANI桥接: ArkUIAniModule._GetPageRootNode], getPageInfoByUniqueId:1307-1318 [ANI桥接: ArkUIAniModule._CustomNode_QueryNavDestinationInfo1 / _CustomNode_QueryRouterPageInfo1], isEasySplit:1496-1498 [ANI桥接: ArkUIAniModule._Common_IsEasySplit]
**动态前端实现**: jsUIContext.js getWindowWidthBreakpoint:950 (withInstanceId → getUINativeModule().common.getWindowWidthBreakpoint), getWindowHeightBreakpoint:956 (withInstanceId → getUINativeModule().common.getWindowHeightBreakpoint), getPageRootNode:1104 (isAvailable 校验 + withInstanceId → getUINativeModule().getPageRootNode), getPageInfoByUniqueId:877 (withInstanceId → getUINativeModule().getPageInfoByUniqueId), isEasySplit:1119 (withInstanceId → Context.isEasySplit), setResourceManagerCacheMaxCountForHSP:486 (静态, 无 withInstanceId, getUINativeModule().resource.setResourceManagerCacheMaxCountForHSP)

#### 直接方法动态前端实现补充

**动态前端实现**: jsUIContext.js animateTo:626 (withInstanceId → Context.animateTo), animateToImmediately:797 (withInstanceId → Context.animateToImmediately), keyframeAnimateTo:791 (withInstanceId → Context.keyframeAnimateTo), createAnimator:505 (withInstanceId + NAPI requireNapi('animator')), showAlertDialog:632 (withInstanceId → AlertDialog.show), showActionSheet:638 (withInstanceId → ActionSheet.show), showDatePickerDialog:703 (withInstanceId → DatePickerDialog.show), showTimePickerDialog:709 (withInstanceId → TimePickerDialog.show), showTextPickerDialog:715 (withInstanceId → TextPickerDialog.show), openBindSheet:644 (withInstanceId → Context.openBindSheet + getNodePtr()), updateBindSheet:670 (withInstanceId → Context.updateBindSheet + getNodePtr()), closeBindSheet:689 (withInstanceId → Context.closeBindSheet), vp2px:559 (withInstanceId → globalThis.vp2px), px2vp:565 (withInstanceId → globalThis.px2vp), fp2px:571 (withInstanceId → globalThis.fp2px), px2fp:577 (withInstanceId → globalThis.px2fp), lpx2px:583 (withInstanceId → globalThis.lpx2px), px2lpx:589 (withInstanceId → globalThis.px2lpx), getFrameNodeById:820 (withInstanceId → getUINativeModule().getFrameNodeByKey), getFrameNodeByUniqueId:862 (withInstanceId → getUINativeModule().getFrameNodeByUniqueId)
**静态前端实现**: @ohos.arkui.UIContext.ts animateTo [ANI桥接: ArkUIAniModule动画闭包派发], animateToImmediately [ANI桥接: ArkUIAniModule动画立即派发], keyframeAnimateTo [ANI桥接: ArkUIAniModule关键帧动画], createAnimator [ANI桥接: NAPI animator.create], showAlertDialog [ANI桥接: ArkUIAniModule._PromptAction_showToast/showDialog], showActionSheet [ANI桥接: ArkUIAniModule._PromptAction_showActionMenu], showDatePickerDialog/showTimePickerDialog/showTextPickerDialog [ANI桥接: ArkUIAniModule._DatePicker/TimePicker/TextPickerDialog], openBindSheet/updateBindSheet/closeBindSheet [ANI桥接: ArkUIAniModule._BindSheet_*], vp2px/px2vp/fp2px/px2fp/lpx2px/px2lpx [ANI桥接: ArkUIAniModule._Common_vp2px/px2vp/fp2px/px2fp/lpx2px/px2lpx], getFrameNodeById [ANI桥接: ArkUIAniModule._FrameNode_getFrameNodeById], getFrameNodeByUniqueId [ANI桥接: ArkUIAniModule._FrameNode_getFrameNodeByUniqueId]
