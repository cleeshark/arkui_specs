...
UIContext 是 ArkUI 框架的多实例上下文门面（Facade），为路由、弹窗、动画、拖拽等 UI 能力提供统一入口和实例路由基础。本 Feat 覆盖 UIContext 入口架构（纯虚接口 + UIContextImpl 委托门面）与实例路由机制（ContainerScope RAII、Sync/Restore、C-API scope），合并原 Feat-01（入口与委托架构）与原 Feat-02（构造器、实例身份查询、实例路由基础设施）中的"入口架构 + 实例路由"内容。不含 resolveUIContext/window-free API 及 getXxx 工厂方法（归属 Feat-02/03）。Kit::UIContext 纯虚接口与 UIContextImpl 委托实现仅在本文做概述级描述，不独立展开为特性规格。

**架构要点**：
- Kit 层：UIContext（纯虚接口）→ UIContextImpl（持有 PipelineContext* context_，全委托，不持业务状态）
- ArkTS 静态前端：UIContext 类持有 instanceId_: int32 + 18 delegate 子对象，每个方法 Sync_InstanceId/Restore_InstanceId 配对（ArkUIAniModule → ani_api.h 函数指针 → ContainerScope）
- ArkTS 动态前端：jsUIContext UIContext 类持有 instanceId_ + 惰性 getter delegate 子对象，每个方法 withInstanceId → __JSScopeUtil__.syncInstanceId/restoreInstanceId（js_scope_util.cpp → ContainerScope）；缓存由 UIContextHelper(JSI) uiContextMap_ + __getUIContext__ 管理
- C-API：ArkUI_ContextHandle（即 ArkUI_Context*，内部仅 { int32_t id }）轻量实例路由令牌
- 实例路由：Kit 层 ContainerScope RAII；ArkTS 静态前端 Sync_InstanceId / Restore_InstanceId；ArkTS 动态前端 withInstanceId + __JSScopeUtil__；C-API OH_ArkUI_RunTaskInScope
...
graph TD
    ...
    subgraph ArkTS动态前端
        JsUIContext[jsUIContext UIContext 类]
        UIContextHelper[UIContextHelper uiContextMap_缓存]
        JSScopeUtil[JSScopeUtil __JSScopeUtil__]
        WithInstanceId[withInstanceId Sync/Restore]
        JsUIContext -->|instanceId_| UIContextHelper
        JsUIContext -->|每个delegate方法| WithInstanceId
        WithInstanceId -->|__JSScopeUtil__.syncInstanceId| JSScopeUtil
    end
    subgraph C-API层
...
| 类型 | 内容 | 说明 |
|------|------|------|
| ADDED | UIContext 纯虚接口 + UIContextImpl 委托实现（Kit 层概述，不独立展开）、ArkTS UIContext 类、C-API ArkUI_Context 结构体 | 类定义与委托门面架构 |
...
| ADDED | UIContextHelper (JSI) uiContextMap_ 缓存与 getOrCreateUIContextById | ArkTS 动态前端缓存门面（静态前端对应: UIContextUtil.contextMap_ + ANI resolveUIContext 函数指针，见R-40） |
| ADDED | JSScopeUtil (__JSScopeUtil__) SyncInstanceId/RestoreInstanceId/GetCallingScopeUIContext | ArkTS 动态前端实例路由桥（静态前端对应: ArkUIAniModule._Common_Sync/Restore_InstanceId + ani_api.h 5函数指针，见R-41） |
| ADDED | UIContextUtil 全量 12 方法 (getOrCreateCurrentUIContext/getCurrentInstanceId/getOrCreateUIContextById/addAvailableInstanceId/removeAvailableInstanceId/addUIContext/removeUIContext/getUIContextById/checkCurrentThread/resolveUIContext/setEnvContextById/getEnvContextById) UIContextUtil.d.ets 类型声明) UIContextUtil.d.ets 类型声明 | 静态前端实例缓存与路由管理 |
...
| `UIContextUtil.d.ets:19-34` | 当前 | UIContextUtil 内部 API 类型声明 |
| `ani_api.h:685-689` | 当前 | ANI 原生接口 5 函数指针 (getCallingScopeUIContext/getLastFocusedUIContext/getLastForegroundUIContext/getAllInstanceIds/resolveUIContext) |
| `arkoala_api.h:9725` | 当前 | Arkoala runScopedTask 函数指针 |
...
| AC-4.3 | US-4 | WHEN ArkTS 静态前端 delegate 方法调用 THEN _Common_Sync_InstanceId(instanceId_) 切换 currentId_，调用后 _Common_Restore_InstanceId() 恢复；WHEN ArkTS 动态前端 delegate 方法调用 THEN withInstanceId(instanceId_) 内部 __JSScopeUtil__.syncInstanceId(instanceId_) 切换 currentId_，finally 中 __JSScopeUtil__.restoreInstanceId() 恢复 |
...
动态前端对应：jsUIContext.js:2036-2043 `withInstanceId(instanceId, callback)` 函数，内部 `__JSScopeUtil__.syncInstanceId(instanceId)` + `finally { __JSScopeUtil__.restoreInstanceId() }` 配对，js_scope_util.cpp:55-71（SyncInstanceId → ContainerScope::UpdateCurrent），js_scope_util.cpp:73-94（RestoreInstanceId → ContainerScope::UpdateCurrent(restoreId)）。
...
动态前端对应：UIContextHelper::uiContextMap_ (ui_context_helper.h:34, ui_context_helper.cpp:41-54) 按 instanceId 缓存 JSValueRef Global 引用，GetUIContext 查缓存不存在时建立 ContainerScope(instanceId) 并调用 ArkTSUtils::GetContext(vm) 创建 UIContext 后存入；jsUIContext.js:2050-2052 `__getUIContext__(instanceId)` → `new UIContext(instanceId)` 创建实例。
...
动态前端对应：jsUIContext.js:425-427 `constructor(instanceId) { this.instanceId_ = instanceId; }`，构造器传入 instanceId（由 __getUIContext__(instanceId) 从 C++ 侧注入），jsUIContext.js:595-597 `getId()` 返回 `this.instanceId_ ?? -1`。
...
静态前端对应：UIContextUtil.getOrCreateUIContextById(id) 先查 contextMap_，不存在则 new UIContext(id) 并存入（UIContextUtil.ets:33-40），与 UIContextHelper::GetUIContext 行为语义一致但桥接路径不同（见R-40）。
...
静态前端对应：ani_api.h:685-689 的 5 函数指针（getCallingScopeUIContext/getLastFocusedUIContext/getLastForegroundUIContext/getAllInstanceIds/resolveUIContext）与 JSScopeUtil 的 7 NAPI 方法行为语义一致但调用路径不同（静态前端通过 ANI 函数指针 → ContainerScope，动态前端通过 __JSScopeUtil__ → js_scope_util NAPI → ContainerScope，见R-41）。
...
动态前端对应：jsUIContext.js:425-427 构造器仅存储 `this.instanceId_ = instanceId`，delegate 子对象通过惰性 getter 创建（Pattern A: 每次创建新对象如 Router:500-503, Font:495-498；Pattern B: null-check 惰性创建如 FocusController:889-894, ComponentUtils:599-604），每个 delegate 方法内部通过 `withInstanceId(this.instanceId_, () => {...})` 确保实例路由。

### R-33: UIContextUtil.getOrCreateCurrentUIContext() 惰性创建当前实例 [行为]

UIContextUtil.getOrCreateCurrentUIContext() 调用 UIContextHelper::GetUIContext(context_->GetInstanceId()) 获取当前实例 UIContext，若不存在则通过 ArkTSUtils::GetContext(vm) 创建并缓存到 contextMap_。

来源验证：`UIContextUtil.ets:24-28`。

动态前端对应：C++ 侧通过 `JsiDeclarativeEngineInstance::CallGetUIContextFunc()` (jsi_declarative_engine.cpp:1306-1321) 调用 JS 侧 `__getUIContext__(instanceId)` (jsUIContext.js:2050-2052) → `new UIContext(instanceId)`，缓存由 UIContextHelper::uiContextMap_ 管理。

### R-34: UIContextUtil.getCurrentInstanceId() 返回当前实例 ID [行为]

UIContextUtil.getCurrentInstanceId() 调用 ContainerScope::CurrentId() 获取当前线程的实例 ID。

来源验证：`UIContextUtil.ets:29-30`。

动态前端对应：C++ 侧 `JsiDeclarativeEngineInstance::instanceId_` (jsi_declarative_engine.cpp:1547) 直接暴露当前实例 ID，无独立 JS 方法。

### R-35: UIContextUtil.addAvailableInstanceId/removeAvailableInstanceId 注册/注销可用性 [行为]

addAvailableInstanceId 调用 UIContextHelper::AddUIContext(instanceId) 将 ID 加入 contextMap_；removeAvailableInstanceId 调用 UIContextHelper::RemoveUIContext(instanceId) 从 contextMap_ 移除。

来源验证：`UIContextUtil.ets:45-61, UIContextUtil.ets:61-67`。

动态前端对应：jsUIContext.js:2081 `__availableInstanceIds__ = new Set()`；jsUIContext.js:2087-2089 `__addAvailableInstanceId__(instanceId) → __availableInstanceIds__.add(instanceId)`；jsUIContext.js:2095-2097 `__removeAvailableInstanceId__(instanceId) → __availableInstanceIds__.delete(instanceId)`；C++ 侧 `CallAddAvailableInstanceIdFunc` (jsi_declarative_engine.cpp:1433-1442) 和 `CallRemoveAvailableInstanceIdFunc` (jsi_declarative_engine.cpp:1444-1453) 调用这些全局函数。

### R-36: UIContextUtil.addUIContext/removeUIContext/getUIContextById 缓存增删查 [行为]

addUIContext 调用 contextMap_.set(instanceId, uiContext) 存入实例缓存；removeUIContext 调用 contextMap_.delete(instanceId) 移除；getUIContextById 调用 contextMap_.get(instanceId) 查询缓存。

来源验证：`UIContextUtil.ets:73-40, UIContextUtil.ets:77-78, UIContextUtil.ets:81-82`。

动态前端对应：UIContextHelper::AddUIContext/RemoveUIContext/HasUIContext (ui_context_helper.cpp:26-61) 管理 uiContextMap_，与 UIContextUtil addUIContext/removeUIContext/getUIContextById 行为语义一致。

### R-37: UIContextUtil.checkCurrentThread() 线程安全检查 [边界]

checkCurrentThread 调用 Container::Current() 检查当前线程是否为 UI 线程，非 UI 线程返回 false。

来源验证：`UIContextUtil.ets:87-88`。

动态前端对应：jsUIContext.js:1061-1063 `isAvailable()` 内部 `__availableInstanceIds__.has(this.instanceId_)` 检查可用性；C++ 侧 Container::Current() 检查当前线程是否为 UI 线程。

### R-38: UIContextUtil.resolveUIContext() 静态前端多实例解析 [行为]

UIContextUtil.resolveUIContext() 调用 ani_api.h 的 resolveUIContext 函数指针，返回 Array<int>（instanceId + reason 二元组）。

来源验证：`UIContextUtil.ets:91`。

动态前端对应：jsUIContext.js:464-467 `UIContext.resolveUIContext()` → `__JSScopeUtil__.resolveUIContext()` → js_scope_util.cpp:126-134 `JSScopeUtil::ResolveUIContext` → `ContainerScope::CurrentIdWithReason()` 返回 [instanceId, reason] 二元组。

### R-39: UIContextUtil.setEnvContextById/getEnvContextById 环境上下文存取 [行为]

setEnvContextById 将环境属性值存入 envContextMap_；getEnvContextById 从 envContextMap_ 查询。

来源验证：`UIContextUtil.ets:94-108`。

动态前端对应：ace_engine 动态前端代码中未发现 setEnvContextById/getEnvContextById 的 JS 或 C++ 实现，推测此功能仅在静态前端通过 UIContextUtil 提供。

### R-40: UIContextUtil 与 UIContextHelper 行为一致性 [边界]

UIContextUtil 的静态前端方法（getOrCreateUIContextById/addAvailableInstanceId/removeAvailableInstanceId）与 UIContextHelper 的动态前端方法（GetUIContext/AddUIContext/RemoveUIContext）行为语义一致，但桥接层不同：静态前端通过 ANI 函数指针，动态前端通过 JSI/NAPI。

来源验证：`UIContextUtil.ets:33-40`, `ui_context_helper.h:25-36`, `ani_api.h:685-689`。

### R-41: ANI 原生接口函数指针桥接一致性 [边界]

ani_api.h 的 5 函数指针 (getCallingScopeUIContext/getLastFocusedUIContext/getLastForegroundUIContext/getAllInstanceIds/resolveUIContext) 与 js_scope_util.cpp 的 7 NAPI 方法行为语义一致，但调用路径不同：静态前端通过 ANI 函数指针 → ContainerScope，动态前端通过 __JSScopeUtil__ → js_scope_util NAPI → ContainerScope。arkoala_api.h 的 runScopedTask 函数指针与 jsUIContext.js 的 withInstanceId + __JSScopeUtil__.syncInstanceId 行为语义一致。

来源验证：`ani_api.h:685-689`, `arkoala_api.h:9725`, `js_scope_util.h:25-38`。
...
| VM-9 | 代码审查 + ArkTS 集成测试 | US-4/AC-4.3/R-8 | 静态前端 Sync_InstanceId/Restore_InstanceId 配对（ArkUIAniModule.ts:124-126）；动态前端 withInstanceId + __JSScopeUtil__.syncInstanceId/restoreInstanceId 配对（jsUIContext.js:2036-2043, js_scope_util.cpp:55-94） |
...
| VM-13 | 代码审查 | US-5/AC-5.2/R-22 | 静态前端 UIContextUtil.contextMap_ 缓存（UIContextUtil.ets:33-40）；动态前端 UIContextHelper.uiContextMap_ 缓存（ui_context_helper.h:34, ui_context_helper.cpp:41-54） |
...
| VM-29 | 代码审查 | R-29 | 动态前端 UIContextHelper JSI 缓存门面（ui_context_helper.h:25-36, ui_context_helper.cpp:41-54）；静态前端对应 UIContextUtil（见R-40） |
| VM-30 | 代码审查 | R-30 | 动态前端 JSScopeUtil __JSScopeUtil__ 桥（js_scope_util.h:25-38, js_scope_util.cpp:42-134）；静态前端对应 ANI 函数指针（见R-41） |
| VM-31 | 代码审查 + 条件编译测试 | R-31 | ENABLE_CONTAINER_SCOPE_TRACKING 诊断模式 |
| VM-32 | 代码审查 | R-32 | 静态前端 UIContext 构造器创建 18+ delegate（@ohos.arkui.UIContext.ts:765-782）；动态前端构造器仅存 instanceId_，delegate 通过惰性 getter 创建（jsUIContext.js:425-427 + 各getter方法） |
| VM-33 | 代码审查 | R-33 | 静态前端 UIContextUtil.getOrCreateCurrentUIContext（UIContextUtil.ets:24-28）；动态前端 C++ CallGetUIContextFunc → __getUIContext__(instanceId)（jsi_declarative_engine.cpp:1306-1321, jsUIContext.js:2050-2052） |
| VM-34 | 代码审查 | R-34 | 静态前端 UIContextUtil.getCurrentInstanceId（UIContextUtil.ets:29-30）；动态前端 C++ JsiDeclarativeEngineInstance::instanceId_（jsi_declarative_engine.cpp:1547） |
| VM-35 | 代码审查 | R-35 | 静态前端 UIContextUtil.addAvailableInstanceId/removeAvailableInstanceId（UIContextUtil.ets:45-67）；动态前端 __addAvailableInstanceId__/__removeAvailableInstanceId__（jsUIContext.js:2087-2089/2095-2097） + C++ CallAdd/RemoveAvailableInstanceIdFunc（jsi_declarative_engine.cpp:1433-1453） |
| VM-36 | 代码审查 | R-36 | 静态前端 UIContextUtil.addUIContext/removeUIContext/getUIContextById（UIContextUtil.ets:73-82）；动态前端 UIContextHelper.AddUIContext/RemoveUIContext/HasUIContext（ui_context_helper.cpp:26-61） |
| VM-37 | 代码审查 | R-37 | 静态前端 UIContextUtil.checkCurrentThread（UIContextUtil.ets:87-88）；动态前端 jsUIContext.js:1061-1063 isAvailable() + Container::Current() 检查线程 |
| VM-38 | 代码审查 | R-38 | 静态前端 UIContextUtil.resolveUIContext → ani_api.h resolveUIContext 函数指针（UIContextUtil.ets:91）；动态前端 UIContext.resolveUIContext → __JSScopeUtil__.resolveUIContext → js_scope_util.cpp:126-134 |
| VM-39 | 代码审查 | R-39 | 静态前端 UIContextUtil.setEnvContextById/getEnvContextById（UIContextUtil.ets:94-108）；动态前端无对应实现（推测仅静态前端提供） |
| VM-40 | 代码审查 | R-40 | UIContextUtil/UIContextHelper 行为一致性 |
| VM-41 | 代码审查 | R-41 | ANI/js_scope_util 桥接一致性 |
...
**静态前端实现**: @ohos.arkui.UIContext.ts:735-799（构造器765-782创建18 delegate子对象，每个方法 Sync/Restore 配对通过 ArkUIAniModule）
**动态前端实现**: jsUIContext.js:418-1131（构造器425-427仅存instanceId_，delegate通过惰性getter创建，每个方法 withInstanceId + __JSScopeUtil__ 配对）
...
**静态前端实现**: UIContextUtil.ets:20-110（静态前端实例缓存管理，contextMap_ + availableInstanceIds_ + envContextMap）
**动态前端实现**: 无独立UIContextUtil类；等效机制通过 C++ CallGetUIContextFunc (jsi_declarative_engine.cpp:1306-1321) → __getUIContext__(instanceId) (jsUIContext.js:2050-2052) + UIContextHelper::uiContextMap_ (ui_context_helper.h:34) + __availableInstanceIds__ (jsUIContext.js:2081) 管理
...
export class UIContextUtil {
    ...
    static getOrCreateCurrentUIContext(): UIContext;           // UIContextUtil.ets:24
    static getCurrentInstanceId(): int32;                      // UIContextUtil.ets:29
    static getOrCreateUIContextById(instanceId: int32): UIContext; // UIContextUtil.ets:33
    static addAvailableInstanceId(instanceId: int32): void;    // UIContextUtil.ets:45
    static removeAvailableInstanceId(instanceId: int32): void; // UIContextUtil.ets:61
    static addUIContext(instanceId: int32, uiContext: UIContext): void; // UIContextUtil.ets:73
    static removeUIContext(instanceId: int32): void;           // UIContextUtil.ets:77
    static getUIContextById(instanceId: int32): UIContext | undefined; // UIContextUtil.ets:81
    static checkCurrentThread(): boolean;                      // UIContextUtil.ets:87
    static resolveUIContext(): Array<int>;                     // UIContextUtil.ets:91
    static setEnvContextById(instanceId: int32, envValue: string, envProperty: Object): void; // UIContextUtil.ets:94
    static getEnvContextById(instanceId: int32, envValue: string): Object | undefined;         // UIContextUtil.ets:103
}
```

来源：`UIContextUtil.d.ets:19-34`（类型声明）

```typescript
export class UIContextUtil {
    static getCurrentInstanceId(): int32;                      // UIContextUtil.d.ets:19
    static getOrCreateCurrentUIContext(): UIContext;           // UIContextUtil.d.ets:22
    static getOrCreateUIContextById(instanceId: int32): UIContext; // UIContextUtil.d.ets:25
    static addAvailableInstanceId(instanceId: int32): void;    // UIContextUtil.d.ets:28
    static removeAvailableInstanceId(instanceId: int32): void; // UIContextUtil.d.ets:31
    static resolveUIContext(): Array<int>;                     // UIContextUtil.d.ets:34
}
```

#### ArkTS 动态前端 Sync/Restore 模式 (withInstanceId + __JSScopeUtil__)

来源：`jsUIContext.js:2036-2043`, `js_scope_util.cpp:55-94`

**静态前端实现**: ArkUIAniModule._Common_Sync_InstanceId / _Common_Restore_InstanceId（ArkUIAniModule.ts:124-126, ani_api.h 函数指针）
**动态前端实现**: withInstanceId(instanceId, callback) 内部 __JSScopeUtil__.syncInstanceId(instanceId) + finally { __JSScopeUtil__.restoreInstanceId() }

```typescript
// jsUIContext.js:2036-2043
...
C++ 桥接：`js_scope_util.cpp:55-71`（SyncInstanceId → push restoreInstanceIds_ + ContainerScope::UpdateCurrent），`js_scope_util.cpp:73-94`（RestoreInstanceId → pop restoreInstanceIds_ + ContainerScope::UpdateCurrent(restoreId)）。
...
**动态前端实现**: ui_context_helper.h:25-36, ui_context_helper.cpp:41-54（JSI层缓存门面，按instanceId缓存JSValueRef Global引用）
**静态前端对应**: UIContextUtil.contextMap_ + getOrCreateUIContextById（UIContextUtil.ets:33-40），见R-40
...
**动态前端实现**: js_scope_util.h:25-38, js_scope_util.cpp:42-134（NAPI层桥接，提供SyncInstanceId/RestoreInstanceId/GetCallingScopeUIContext/ResolveUIContext 7个静态方法）
**静态前端对应**: ani_api.h:685-689 5函数指针 + arkoala_api.h:9725 runScopedTask，见R-41
...
来源：`frameworks/core/interfaces/ani/ani_api.h:685-689`, `frameworks/core/interfaces/arkoala/arkoala_api.h:9725`

ANI 接口结构体包含 5 个 UIContext 实例解析函数指针和 1 个 runScopedTask 函数指针，静态前端通过这些函数指针桥接到 C++ ContainerScope 层。
...
静态前端调用路径：@ohos.arkui.UIContext.ts → ArkUIAniModule → ani_api.h 函数指针 → ContainerScope(id)
动态前端调用路径：jsUIContext.js → __JSScopeUtil__ → js_scope_util.cpp → ContainerScope(id)
...
- [x] 全局特性影响含影响级别列
- [x] 常量/枚举/类型别名完整列出
- [x] ArkTS 静态前端 + JSI 动态前端双桥覆盖（R-8/R-22/R-23/R-29/R-30/R-32/R-33~R-39 补入双侧引用，Mermaid架构图含动态前端子图，接口规格含并行节）
...
jsuicontext_js: frameworks/bridge/declarative_frontend/engine/jsUIContext.js
jsi_declarative_engine_cpp: frameworks/bridge/declarative_frontend/engine/jsi/jsi_declarative_engine.cpp
