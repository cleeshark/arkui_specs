...
静态前端对应: UIContextUtil.resolveUIContext() → ani_api.h resolveUIContext 函数指针 → ContainerScope::CurrentIdWithReason()（UIContextUtil.ets:91, ani_api.h:689）；ArkUIAniModule._ResolveUIContext 返回 KInt 数组（ArkUIAniModule.ts）
...
静态前端对应: @ohos.arkui.UIContext.ts:783 constructor() instanceId_ = -1（无参构造器），@ohos.arkui.UIContext.ts:765-766 constructor(instanceId) instanceId_ = instanceId（有参构造器）
...
静态前端对应: @ohos.arkui.UIContext.ts:854 isAvailable() → UIContextUtil.availableInstanceIds_.has(this.instanceId_)（UIContextUtil.ets:62-67）
...
静态前端对应: @ohos.arkui.UIContext.ts:1073 runScopedTask(callback) → ArkUIAniModule._Common_Sync_InstanceId(instanceId_) + callback + _Common_Restore_InstanceId()（@ohos.arkui.UIContext.ts:1073-1076）
...
静态前端对应: @ohos.arkui.UIContext.ts:975-977 getHostContext() → ArkUIAniModule._Common_Sync_InstanceId(instanceId_) + getContext(this.instanceId_) + _Common_Restore_InstanceId()；@ohos.arkui.UIContext.ts:924-926 getSharedLocalStorage() → 同样 Sync/Restore + NativeLocalStorage.GetShared(this.instanceId_)
...
静态前端对应: ani_api.h:689 resolveUIContext 函数指针同样调用 ContainerScope::CurrentIdWithReason()，但子容器 ID 映射在 C++ 层完成（UIContextUtil.ets:91 → resolveUIContext → ContainerScope::CurrentIdWithReason → GetMainInstanceId）
...
| VM-7 | AC-2.7, R-1 | NAPI 测试 | getCallingScopeUIContext 存在调用域返回 UIContext；动态: jsUIContext.js:429-435 → __JSScopeUtil__.getCallingScopeUIContext → js_scope_util.cpp:96-99；静态: @ohos.arkui.UIContext.ts:812 → ArkUIAniModule._GetCallingScopeUIContext → ani_api.h:685 getCallingScopeUIContext 函数指针 |
| VM-8 | AC-2.8, R-5 | NAPI 测试 | new UIContext() → getId()=-1；动态: jsUIContext.js:595-596；静态: @ohos.arkui.UIContext.ts:783 constructor() instanceId_=-1 |
| VM-9 | AC-2.9, R-1 | 单元测试 | ContainerScope::GetAllUIContexts() 返回 containerSet_；container_scope.h:86 |
| VM-10 | AC-2.10, R-5, R-6 | NAPI 测试 | isAvailable() 对销毁实例返回 false；动态: jsUIContext.js:1061-1062 __availableInstanceIds__.has(instanceId_)；静态: @ohos.arkui.UIContext.ts:854 → UIContextUtil.availableInstanceIds_.has(instanceId_) |
...
| VM-13 | AC-2.13, R-8, R-14 | NAPI 测试 | runScopedTask 在 withInstanceId 作用域内执行 callback；动态: jsUIContext.js:721-726 withInstanceId(instanceId, callback)；静态: @ohos.arkui.UIContext.ts:1073 → ArkUIAniModule._Common_Sync/Restore_InstanceId 配对 |
| VM-14 | AC-2.14, R-12 | NAPI 测试 | getHostContext 返回 getContext()；动态: jsUIContext.js:808-812 withInstanceId + getContext(instanceId)；静态: @ohos.arkui.UIContext.ts:975-977 Sync/Restore + getContext(instanceId) |
| VM-15 | AC-2.15, R-12 | NAPI 测试 | getSharedLocalStorage 返回 NativeLocalStorage.GetShared()；动态: jsUIContext.js:814-818 withInstanceId + NativeLocalStorage.GetShared(instanceId)；静态: @ohos.arkui.UIContext.ts:924-926 Sync/Restore + NativeLocalStorage.GetShared(instanceId) |
...
源码验证: jsUIContext.js:595-596
**动态前端实现**: jsUIContext.js:595
**静态前端实现**: @ohos.arkui.UIContext.ts:1265 [ANI桥接: getInstanceId]
...
**动态前端实现**: jsUIContext.js:938
**静态前端实现**: @ohos.arkui.UIContext.ts:1202
...
**动态前端实现**: jsUIContext.js:944
**静态前端实现**: @ohos.arkui.UIContext.ts:1205-1211
...
**动态前端实现**: jsUIContext.js:1061
**静态前端实现**: @ohos.arkui.UIContext.ts:854
...
**动态前端实现**: jsUIContext.js:425-427 constructor(instanceId) { this.instanceId_ = instanceId; }（C++ CallGetUIContextFunc 注入有效 instanceId；无参构造在 C++ 侧不触发）
**静态前端实现**: @ohos.arkui.UIContext.ts:783 constructor() instanceId_ = -1; @ohos.arkui.UIContext.ts:765-766 constructor(instanceId) instanceId_ = instanceId
...
**动态前端实现**: jsUIContext.js:464
**静态前端实现**: @ohos.arkui.UIContext.ts:718-733 [ANI桥接: resolveUIContext]
...
**动态前端实现**: jsUIContext.js:429
**静态前端实现**: @ohos.arkui.UIContext.ts:812 [ANI桥接: getCallingScopeUIContext]
...
**动态前端实现**: jsUIContext.js:437
**静态前端实现**: @ohos.arkui.UIContext.ts:821 [ANI桥接: getLastFocusedUIContext]
...
**动态前端实现**: jsUIContext.js:445
**静态前端实现**: @ohos.arkui.UIContext.ts:830 [ANI桥接: getLastForegroundUIContext]
...
**动态前端实现**: jsUIContext.js:453
**静态前端实现**: @ohos.arkui.UIContext.ts:839 [ANI桥接: getAllInstanceIds]
...
**静态前端实现**: @ohos.arkui.UIContext.ts:726-733
**动态前端实现**: jsUIContext.js:464-467 resolveUIContext() → __JSScopeUtil__.resolveUIContext() → js_scope_util.cpp:126-134 返回 [instanceId, reason] 二元组，reason 值与 ResolveStrategy 一一对应
...
**动态前端实现**: jsUIContext.js:1133
**静态前端实现**: @ohos.arkui.UIContext.ts:718-724
...
**动态前端实现**: jsUIContext.js:469
**静态前端实现**: @ohos.arkui.UIContext.ts:1281
...
**动态前端实现**: jsUIContext.js:479
**静态前端实现**: @ohos.arkui.UIContext.ts:1300
...
源码验证: jsUIContext.js:721-726, jsUIContext.js:2036-2043
**动态前端实现**: jsUIContext.js:721
**静态前端实现**: @ohos.arkui.UIContext.ts:1073 [ANI桥接: arkoala runScopedTask]
...
**动态前端实现**: jsUIContext.js:808
**静态前端实现**: @ohos.arkui.UIContext.ts:975-977
...
**动态前端实现**: jsUIContext.js:814
**静态前端实现**: @ohos.arkui.UIContext.ts:924-926
