...
| 关联 Epic | 无 |
| 目标 Feature | Feat-01 状态效果, Feat-02 动态属性设置（attributeModifier） |
| 复杂度 | 中等 |
...
| 项 | 内容 |
|----|------|
| 问题陈述 | 开发者需要通过声明式 API 控制组件的交互状态效果与反馈样式 |
| 核心目标 | （Feat-01）提供 stateStyles/hoverEffect/clickEffect 状态效果属性，支持按压/悬停/聚焦/禁用等状态样式和交互反馈效果 |
| P0 AC | （Feat-01）stateStyles 按状态应用样式覆盖；hoverEffect/clickEffect 交互反馈效果；（Feat-02）attributeModifier 按状态回调修改属性；excludeInner 抑制系统默认样式 |
...
|------|-----------|----------|-----------------|
| ace_engine | `frameworks/core/components_ng/pattern/pattern.cpp` | Pattern 处理 stateStyles 状态切换和样式应用 | 状态效果管线 |
...
| ace_engine | `frameworks/core/components_ng/base/view_abstract.h/cpp` | API 入口，SetStateStyles/SetHoverEffect/SetClickEffect | API 层 |
| ace_engine | `frameworks/bridge/declarative_frontend/jsview/js_view_abstract.cpp` | ArkTS 桥接层，JsStateStyles/JsHoverEffect 参数解析 | 输入校验 |
...
| 层 | 模块 | 职责 | 修改类型 |
|----|------|------|----------|
| JS Bridge | `declarative_frontend/jsview/js_view_abstract` | 解析 ArkTS 属性调用（JsStateStyles/JsHoverEffect 等），参数校验 | 存量分析 |
...
| API 层 | `core/components_ng/base/view_abstract` | 框架属性设置统一入口（SetStateStyles/SetHoverEffect/SetClickEffect），更新 Pattern/StateStyleManager | 存量分析 |
...
|---------|----------|----------|----------|
| OH-ARCH-LAYERING | 状态效果涉及 JS Bridge → API 层 → Property 层单向调用 | JS Bridge → Pattern/StateStyleManager，严格单向 | 代码评审 |

## 不涉及项承接

| 维度 | 需求阶段结论 | 设计阶段处理方式 | 设计结论 |
|------|---------|-------------|----------|
| IPC/跨进程 | N/A | 保持 N/A | 状态效果仅在 UI 线程内处理 |
| 安全与权限 | N/A | 保持 N/A | 无权限要求 |
| 构建与部件 | N/A | 保持 N/A | 无新增部件或 target |
| 兼容性 | 涉及 | 展开设计 | HoverEffectType.OPACITY 在 NG 管线不生效；attributeModifier 与 stateStyles 互斥 |
...
|---------|------|----------|-----------------|------|------|
| ADR-S1 | stateStyles 状态样式覆盖链 | StateStyleManager 按组件状态（normal/pressed/focused/disabled/selected/hovered）切换样式，由 Pattern 负责状态检测 | 方案A：布尔值开关（无法区分状态）；方案B：直接属性覆盖（无法按状态分组） | 状态样式按 6 种状态分组覆盖，开发者可精确控制不同交互状态的视觉表现 | stateStyles 切换时样式覆盖生效 |
...
| 骨架项 | 目标 | 不包含 | 验证方式 |
|--------|------|--------|----------|
| 状态效果管线 | stateStyles→StateStyleManager, hoverEffect/clickEffect→Pattern | 各状态样式的具体属性值 | 状态切换时样式覆盖正确 |
...
| Task ID | 目标 | 受影响文件 | AC |
|---------|------|------------|-----|
| TASK-SKELETON-1 | 状态效果属性存储与切换 | view_abstract.h, state_style_manager.h, pattern.cpp | WHEN stateStyles 设置 THEN 状态切换时样式覆盖正确 |
...
| Feat-01-state-effect-spec.md | 固化 stateStyles/hoverEffect/clickEffect 的行为规格 | 本 Design | 完整行为规格与 AC |
...
graph TB
    ...
    subgraph PROP["Property 层"]
        ...
        PATTERN["Pattern · hoverEffect/clickEffect"]
    end

    F1 -->|"SetStateStyles"| SSM
    F1 -->|"SetHoverEffect/SetClickEffect"| PATTERN
```
...
| 项 | 类型 | 影响 | 处理方式 | Owner |
|----|------|------|----------|-------|
| hoverEffect/clickEffect 实现细节 | 文档 | 低 | hoverEffect 仅在部分组件生效，clickEffect 规格需后续补充详细参数 | ArkUI SIG |
| HoverEffectType.OPACITY 在 NG 管线不生效 | 版本 | 中 | OPACITY 仅旧管线 RenderBox 实现，NG 管线 FrameNode::AnimateHoverEffect 不处理 | ArkUI SIG |
