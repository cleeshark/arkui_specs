...
| AC-3.1~AC-3.7 | R-3, R-15~R-18, R-29~R-31, R-37 | 已有实现 | 单测/源码审查 | `js_view_abstract.cpp:6124`, `js_view_abstract.cpp:6242`, `view_abstract.cpp:5101`, `view_abstract.cpp:5192`, `node_common_modifier.cpp:2196`, `node_common_modifier.cpp:2402`, `style_modifier.cpp:1017` |
| AC-4.1~AC-4.7 | R-4, R-19~R-22, R-32~R-33, R-38 | 已有实现 | 单测/源码审查 | `js_view_abstract.cpp:6150`, `js_view_abstract.cpp:6271`, `js_view_abstract.cpp:6308`, `node_common_modifier.cpp:2805`, `node_common_modifier.cpp:4328`, `rosen_render_context.cpp:5659`, `rosen_render_context.cpp:5327` |
| AC-5.1~AC-5.4 | R-5, R-23, R-34, R-39 | 已有实现 | SDK/C API 审查 | `common.d.ts:25580`, `common.static.d.ets:11853`, `native_node.h:365`, `native_node.h:2001`, `node_common_modifier.cpp:2223`, `node_common_modifier.cpp:2805`, `node_common_modifier.cpp:4328` |
...
|--------|------|----------|----------|-----------|--------|
| R-1 | 行为 | ArkTS dynamic/static、Native public `NODE_OPACITY` | `opacity` 控制组件整体 alpha，外部契约范围为 `[0,1]` | — | AC-1.1~AC-1.6 |
| R-2 | 行为 | 适用于 brightness/contrast/grayscale/colorBlend/saturate/sepia/invert/hueRotate | 基础滤镜均为前景图像效果，最终由 Rosen 对 RSNode 设置对应滤镜属性 | — | AC-2.1~AC-2.8 |
| R-3 | 行为 | 前景/背景属性分层 | `blur` 是内容/前景模糊，`backdropBlur` 是背景模糊，两者存储和更新路径不同 | — | AC-3.1~AC-3.7 |
| R-4 | 行为 | dynamic/static API，内部 common modifier | `linearGradientBlur` 和 `motionBlur` 属于模糊特效增强能力，参数非法时回退当前实现定义的默认值或 return | — | AC-4.1~AC-4.7 |
| R-5 | 行为 | dynamic/static API 和 Native C API | 外部 API 签名以 SDK 类型定义为准，源码偏差必须记录为风险 | — | AC-5.1~AC-5.4 |
| R-6 | 行为 | 设置 opacity | `opacity` 写入 RenderContext 独立 `Opacity` 属性，Rosen `OnOpacityUpdate` 更新 alpha | RenderContext/RosenRenderContext | AC-1.1 |
| R-7 | 行为 | 设置 brightness | `brightness` 写入 `FrontBrightness`，Rosen 调用 `SetBrightness` | GraphicsProperty | AC-2.1 |
| R-8 | 行为 | 设置或 reset | `contrast/saturate` reset 为 `1.0`，负值在 ArkTS 实现中 clamp 为 `0` | GraphicsProperty | AC-2.3 |
| R-9 | 行为 | 设置或 reset | `grayscale` 范围归一到 `[0,1]`，reset 为 `0` | GraphicsProperty | AC-2.4 |
| R-10 | 行为 | 设置或 reset | `sepia` 负值归一为 `0`，reset 为 `0` | GraphicsProperty | AC-2.5 |
| R-11 | 行为 | 设置 invert | `invert` 支持 number 和 `InvertOptions`，Rosen 分别调用 `SetInvert` 或 `SetAiInvert` | GraphicsProperty/RosenRenderContext | AC-2.6 |
| R-12 | 行为 | 设置 hueRotate | `hueRotate` 角度按 360 取模，负角度加 360 | GraphicsProperty | AC-2.7 |
| R-13 | 行为 | 设置 colorBlend | `colorBlend` reset 和 API >= 12 无效输入均写透明色 | GraphicsProperty | AC-2.8 |
| R-14 | 行为 | 任一基础滤镜变更 | 基础滤镜在 `ViewAbstract` 入口写入 `GraphicsProperty` | RenderContext | AC-2.1~AC-2.8 |
| R-15 | 行为 | 设置 blur | `blur` 写入前景模糊半径和 sysOptions，并更新 front blur filter | ForegroundProperty/RosenRenderContext | AC-3.1~AC-3.3 |
| R-16 | 行为 | 设置 backdropBlur | `backdropBlur` 写入背景模糊半径和 sysOptions，并更新 back blur filter | BackgroundProperty/RosenRenderContext | AC-3.4~AC-3.7 |
| R-17 | 行为 | 设置 backdropBlur | 设置 `backdropBlur` 会清理 `backgroundEffect` 和 `backgroundBlurStyle` | RenderContext | AC-3.5 |
| R-18 | 行为 | 设置 blur | 设置 `blur` 会清理 `foregroundBlurStyle` | RenderContext | AC-3.6 |
| R-19 | 行为 | 设置 linearGradientBlur | `linearGradientBlur` 使用半径、fractionStops 和 direction 构造 `LinearGradientBlurPara` | GraphicsProperty/RosenRenderContext | AC-4.1~AC-4.4 |
| R-20 | 行为 | 设置 motionBlur | `motionBlur` 使用 radius 与 anchor 构造 `MotionBlurOption` | ForegroundProperty/RosenRenderContext | AC-4.5~AC-4.7 |
| R-21 | 行为 | Native `NODE_*` 设置 | Native public `style_modifier` 在进入 common modifier 前执行参数校验 | C API 入口 | AC-1.5, AC-2.9, AC-3.7 |
| R-22 | 行为 | reset common modifier | Native common modifier reset 使用具体无效果值恢复属性 | ViewAbstract/RenderContext | AC-1.6, AC-3.3, AC-4.4, AC-4.7 |
| R-23 | 行为 | 文档补录 | 未在 `native_node.h` 找到的内部 common modifier 能力不得写成公开 C API | API 规格 | AC-5.2, AC-5.4 |
| R-24 | 异常 | target API >= 11 或 < 11 | opacity 越界版本差异 | >= 11 clamp；< 11 回退 `1.0` | AC-1.3, AC-1.4 |
| R-25 | 异常 | `brightness(undefined)` 或不可解析 | brightness dynamic SDK 与 JS 实现差异 | SDK Optional 注释称 reset 到 `0`；当前 JS 实现解析失败写 `1.0` | AC-2.2, AC-5.3 |
| R-26 | 异常 | 值小于 `0` 或大于等于/大于 `1` | grayscale 越界 | ArkTS clamp 到 `[0,1]`；Native public 入口返回错误码 | AC-2.4, AC-2.9 |
| R-27 | 异常 | target API >= 12 或 < 12 | colorBlend 解析失败版本差异 | >= 12 写透明；< 12 return | AC-2.8 |
| R-28 | 异常 | `style_modifier` 判定非法 | Native public 基础滤镜越界 | 返回 `ERROR_CODE_PARAM_INVALID` | AC-2.9 |
| R-29 | 异常 | ArkTS dynamic `blur` | blur 参数缺失或不可解析 | return，不写入新值 | AC-3.2 |
| R-30 | 异常 | target API >= 12 或 < 12 | backdropBlur 参数不可解析版本差异 | >= 12 以 0 继续；< 12 return | AC-3.4 |
| R-31 | 异常 | 参数个数不在 1~3 或灰度不在 `[0,127]` | Native public backdropBlur 非法 | 返回 `ERROR_CODE_PARAM_INVALID` | AC-3.7 |
| R-32 | 异常 | stops 不足或位置非递增 | linearGradientBlur stops 非法 | 回退默认 stops `[(0,0),(0,1)]` | AC-4.2 |
| R-33 | 异常 | 非 object 或半径/anchor 越界 | motionBlur 参数非法 | 非 object return；半径为 0；anchor clamp | AC-4.5, AC-4.6 |
| R-34 | 异常 | `native_node.h` 未找到同级枚举 | 公开 C API 未覆盖全部内部 modifier | 规格标注为内部 modifier 覆盖，不作为 Public C API | AC-5.4 |
| R-35 | 恢复 | opacity reset 或解析失败 | 写入 `1.0`，资源对象路径移除 `viewAbstract.opacity` | 组件恢复完全不透明 | — |
| R-36 | 恢复 | 基础滤镜 reset | 按无效果值恢复：brightness/contrast/saturate=`1`，grayscale/sepia/invert/hueRotate=`0`，colorBlend=透明 | 组件恢复无对应滤镜效果 | — |
| R-37 | 恢复 | blur/backdropBlur reset | 写入 `0px` 半径和默认 `BlurOption` | 前景/背景模糊关闭 | — |
| R-38 | 恢复 | linearGradientBlur/motionBlur reset | linearGradientBlur 写 `0px + 默认 stops + BOTTOM`；motionBlur 写 radius/x/y 为 `0` | 特效关闭 | — |
| R-39 | 恢复 | Native public 参数非法 | 返回错误码，不进入属性写入 | 现有属性不被本次非法调用更新 | — |
