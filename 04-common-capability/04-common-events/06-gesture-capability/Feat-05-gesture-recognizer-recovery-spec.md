...
| AC ID | WHEN/THEN |
|-------|-----------|
| AC-1.1 | WHEN 新一轮首个 down 到达 THEN 恢复非 READY 非豁免残留状态 |
| AC-1.2 | WHEN 识别器处于 READY 或已闭环 THEN 不触发额外恢复和业务回调 |
| AC-2.1 | WHEN 双击识别器处于 pending THEN 豁免恢复 |
| AC-2.2 | WHEN Swipe 识别器处于 pending THEN 豁免恢复 |
| AC-3.1 | WHEN 恢复行为发生 THEN 可测可观测 |
